...
//---- 
	...
	// Channel is Green if width is greater than ChannelAlert (Default 50), otherwise it is Red
      if(ChannelWidth >= ChannelAlert) {ObjectSet("TL1",OBJPROP_COLOR,Blue);}
	      else {ObjectSet("TL1",OBJPROP_COLOR,Blue); }
		ObjectSet("TL1",OBJPROP_WIDTH,2); 
		ObjectSet("TL1",OBJPROP_STYLE,STYLE_SOLID); 
	ObjectCreate("TL2",OBJ_TREND,0,T2,P2,T1,P1); 	
		if(ChannelWidth >= ChannelAlert) {ObjectSet("TL2",OBJPROP_COLOR,Blue); }
   	  else{ObjectSet("TL2",OBJPROP_COLOR,Blue);}
	 ...
	 // Mid line is Green if Slope is greater than SlopeAlert (Default 25), otherwise it is Red
		if(ChannelWidth >= ChannelAlert && MathAbs(SlopeAngle) >= SlopeAlert) {ObjectSet("MIDL",OBJPROP_COLOR,Blue); }
		   else{ObjectSet("MIDL",OBJPROP_COLOR,Blue);}
