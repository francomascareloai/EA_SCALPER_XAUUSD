...
int start()
{
  ...
  {
     int result=0;
     if((Close[0]>iMA(NULL,0,200,0,MODE_SMA,PRICE_CLOSE,0))&&(iMACD(NULL,0,12,26,9,PRICE_CLOSE,MODE_MAIN,0)>iMACD(NULL,0,12,26,9,PRICE_CLOSE,MODE_SIGNAL,0))&&(iADX(NULL,0,14,PRICE_CLOSE,MODE_MAIN,0)>25)) // Here is your open buy rule
     {
     ...
     if((Close[0]<iMA(NULL,0,200,0,MODE_SMA,PRICE_CLOSE,0))&&(iMACD(NULL,0,12,26,9,PRICE_CLOSE,MODE_MAIN,0)<iMACD(NULL,0,12,26,9,PRICE_CLOSE,MODE_SIGNAL,0))&&(iADX(NULL,0,14,PRICE_CLOSE,MODE_MAIN,0)>25)) // Here is your open Sell rule
     {
  ...
  for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if(OrderType()<=OP_SELL &&   
         ...
         )  
        {
         if(OrderType()==OP_BUY)  
           {
              if((Open[0]>Bid)) //here is your close buy rule
              {
           ...
           {
                if((Close[0]>0)) // here is your close sell rule
                {
                   OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,Red);
                }
            if(TrailingStop>0)  
