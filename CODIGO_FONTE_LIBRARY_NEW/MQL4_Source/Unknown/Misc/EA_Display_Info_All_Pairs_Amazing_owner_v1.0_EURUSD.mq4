...
extern color    FontColorPrimary      = White;
extern color    FontColorSecondary    = DarkGray;
extern int      Window                = 0;
extern int      Corner                = 3;
extern int      HorizPos              = 20;
extern int      VertPos               = 4;
extern int      VertSpacing           = 14;
extern string   UniqueID              = "0";
extern int      ADRdays               = 30;
extern int      RefreshEveryXMins     = 0;
extern bool     FullSymbolIDs         = false;
extern bool     ShowBidPrice          = true;
extern bool     ShowDailyMove         = false;
extern bool     ShowADR               = true;
extern bool     ShowSpread            = true;
extern bool     ShowTickval           = true;
extern bool     ShowSwapval           = true;

double   spr, pnt, tickval, bidp, askp, lswap, sswap;
int      dig, tf, ccC;
string   IndiName, ccy, sym, C[40];
datetime prev_time;
...
//+------------------------------------------------------------------+
  ...
  if (StringSubstr(Currencies,StringLen(Currencies)-1,1) != ",")  Currencies = Currencies + ",";
  ccC = StringFindCount(Currencies,",");
  for (int i=0; i<40; i++)
    C[i] = "";
  int comma1 = -1;
  for (i=0; i<40; i++)  {
    int comma2 = StringFind(Currencies,",",comma1+1);
    string temp  = StringSubstr(Currencies,comma1+1,comma2-comma1-1);
    C[i] = ExpandCcy(temp);
    if (comma2 >= StringLen(Currencies)-1)   break;
    comma1 = comma2;
  }  
  ...
  IndiName = "DisplayInfo-" + NumberToStr(checksum,"Z7");
...
//      }
      ...
      for (int k=1; k<=ADRdays; k++)   {
        ...
        sum += (iHigh(ccy,PERIOD_D1,k) - iLow(ccy,PERIOD_D1,k))/pnt;
        cnt += 1;
      }
      double tdy    = (iHigh(ccy,PERIOD_D1,0) - iLow(ccy,PERIOD_D1,0))/pnt;
      double adr    = DivZero(sum,cnt);
      double pctadr = DivZero(100*tdy*cnt,sum);
      double pctspr = DivZero(100*spr*cnt,sum);
      string ast0 = " ", ast1 = " ", ast2 = " ", ast3 = " ";
      if (ccy == sym)    ast0 = "*";
      if (pctadr >= 75)  ast1 = "*";
      if (spr < 3)       ast2 = "*";
      if (pctspr < 2.5)  ast3 = "*";
      if (dig>3) string msk = "':'R2.4"; else msk = "':'R4.2";
      bidp    = MarketInfo(ccy,MODE_BID);
      askp    = MarketInfo(ccy,MODE_ASK);
      double dmove = (bidp-iOpen(ccy,PERIOD_D1,0))/pnt;
      double pmove = 100*(bidp-iOpen(ccy,PERIOD_D1,0))/iOpen(ccy,PERIOD_D1,0);
//---------------------        
//      string tstr1 = ast0 + ReduceCcy(ccy) + NumberToStr(bidp,msk) + NumberToStr(tdy,"R5") + NumberToStr(adr,"':'R3") + NumberToStr(-pctadr,"R(4%") 
//                   + ast1 + NumberToStr(spr,"3.1") + ast2 + NumberToStr(-pctspr,"R(~2.1%") + ast3 + NumberToStr(tickval,"R4.2")
//                   + NumberToStr(lswap,"R+3.2") + NumberToStr(sswap,"R+3.2");
//      string tstr1 = ccy + ": " + NumberToStr(spr,"4.1") + NumberToStr(-100*spr*cnt/sum,"R(3.1%") + NumberToStr(tickval,"R4.2");
//---------------------        
      string tstr1 = ast0 + ReduceCcy(ccy);
      if (FullSymbolIDs)    tstr1 = ast0 + ccy;
      if (ShowBidPrice)     tstr1 = tstr1 + NumberToStr(bidp,msk);
      if (ShowDailyMove)    tstr1 = tstr1 + NumberToStr(dmove,"R+4.1") + NumberToStr(pmove,"R+3.1'%'");
      if (ShowADR)          tstr1 = tstr1 + NumberToStr(tdy,"R5") + NumberToStr(adr,"':'R3") + NumberToStr(-pctadr,"R(4%") + ast1;
      if (ShowSpread)       tstr1 = tstr1 + NumberToStr(spr,"3.1") + ast2 + NumberToStr(-pctspr,"R(~2.1%") + ast3;
      if (ShowTickval)      tstr1 = tstr1 + NumberToStr(tickval,"R4.2");
      if (ShowSwapval)      tstr1 = tstr1 + NumberToStr(lswap,"R+3.2") + NumberToStr(sswap,"R+3.2");
      int yp = VertPos+c*VertSpacing;
      string objname = IndiName + NumberToStr(-i-1,"T-6") + NumberToStr(-j-1,"T-6") ;
      ...
      if (colr == FontColorPrimary)
        colr = FontColorSecondary;
      else
        colr = FontColorPrimary;
        
//      color colr = FontColor;      
//      if (StringFind("EURUSD,GBPUSD,USDCHF,NZDUSD,AUDUSD,USDJPY,USDCAD,NZDJPY,AUDJPY,GBPJPY,EURJPY,",ccy+",",0) < 0)      
//        colr = Gray;      
      ObjectSetText(objname,tstr1,FontSize,FontName,colr);
  } } 
  return(0);
}
