...
extern bool            showSubSwings      = false;          // Display sub-swings?
...
extern bool            alertsNotification = false;          // Alert should send push notification?
extern bool            OnlyCompleted      = false;          // Sho only completed swings/sub-swings
...
int start()
{
   int k,i,counted_bars=IndicatorCounted();
   ...
   for (i=limit; i>=0; i--)
   {
      ...
      int x = iBarShift(NULL,TimeFrame,Time[(int)MathMin(i+1,Bars-1)]);               
      if (x!=y)
      {
         double dummy = iCustom(NULL,TimeFrame,indicatorFileName,PERIOD_CURRENT,showSubSwings,alertsOn,alertsMessage,alertsSound,alertsEmail,alertsNotification,OnlyCompleted,0,y);
            if (dummy!=EMPTY_VALUE)
            {
               if (dummy>iCustom(NULL,TimeFrame,indicatorFileName,PERIOD_CURRENT,showSubSwings,alertsOn,alertsMessage,alertsSound,alertsEmail,alertsNotification,OnlyCompleted,1,y))
               {
                  upBuffer[i]  = High[i];
                  dnBuffer[i]  = Low[i];
                  upsBuffer[i] = MathMax(Open[i],Close[i]);
                  dnsBuffer[i] = MathMin(Open[i],Close[i]);
               }
               else
               {
                  dnBuffer[i]  = High[i];
                  upBuffer[i]  = Low[i];
                  dnsBuffer[i] = MathMax(Open[i],Close[i]);
                  upsBuffer[i] = MathMin(Open[i],Close[i]);
               }                  
            }
            else
            {
...
void manageAlerts()
{
   if (alertsOn)
   {
      ...
      int i; for (i=0; i<Bars; i++) if (swing[i]!=0) break;
