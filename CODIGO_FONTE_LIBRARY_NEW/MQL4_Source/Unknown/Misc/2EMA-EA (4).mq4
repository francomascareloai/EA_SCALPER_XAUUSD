...
//| Input variables                                                  |
//+------------------------------------------------------------------+
input int MaxTrades = 10; // Max Trades (0 for unlimited)
...
int EMA_FAST_PERIOD = 5;
int EMA_SLOW_PERIOD = 20;
int EMA_PRICE_TYPE = PRICE_CLOSE;
...
int OnInit() {
   newtime = 0;
   lastTicket = LastOpenOrderTicket();
...
void OnTick() {
   if(newtime == 0) {
      ...
      if(newtime != Time[0]) {
         ...
         double lastPrice = OrderPrice(lastTicket);
         bool enoughDistanceForNewBuy = MathAbs(lastPrice - Ask) >= spreadDistance * distanceMultiplier;
         bool enoughDistanceForNewSell = MathAbs(lastPrice - Bid) >= spreadDistance * distanceMultiplier;
         ...
         double MinLossThreshold = 10 * spreadDistance;
         ...
         if(emaCrossDn && buy_orders_count > 0) {
            // check the condition to close all buy orders (MinLossThreshold ???)
            CloseOrdersByType(OP_BUY);
            lastTicket = LastOpenOrderTicket();
         }
         if(emaCrossUp && sell_orders_count > 0) {
            // check the condition to close all sell orders (MinLossThreshold ???)
