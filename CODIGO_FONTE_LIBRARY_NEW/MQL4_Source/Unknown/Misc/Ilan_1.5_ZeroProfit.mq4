#property copyright ""
#property link      ""
extern int    PauseMin=10;
extern double LotExponent = 1.8;
extern string s = "максимальное проскальзывание";
extern double slip = 5.0;
extern string r = "размер начального лота в зависимости от эквити";
extern double Risk = 5;
int gi_unused_88;
extern string l = "начальный лот при Risk=0";
extern double Lots = 0.1;
extern string t = "тип счета";
extern int lotdecimal = 2;
// extern string tp = "тейкпрофит до колена Х";
extern double TakeProfit1 = 10.0;
extern string tp2 = "тейкпрофит после колена Х (для выхода в безубыток)";
extern double TakeProfit2 = 1;
extern string ktp = "колено, после которого (на следующем) выставляется ТП2";
extern double KolTakeProfit = 1;
extern double Stoploss = 500.0;
extern double TrailStart = 10.0;
extern double TrailStop = 10.0;
extern string ps = "ограничение шага (ближе ps колена не открываются)";
...
extern string mn = "магический номер";
extern int g_magic_176 = 1966;
// double Vhodmac;
// double Vhodsig; 
// extern string md = "настройки MACD для первого ордера";
// extern double MTF = 0;
// extern int BystrVhod = 9;
// extern int MedlVhod = 12;
// extern int SignVhod = 20;
// extern string st = "настройки стохастика для колен"; 
// extern double MTS = 0;
// extern double Stohmax = 80;
// extern double Stohmin = 20;
double TakeProfit;
// double Kolenomac;
// double Kolenosig; 
...
// extern string k = "коэффициент множителя лота";
// extern double Kor = 15;     // корректирует множитель
extern string w = "работа советника в пятницу до";
...
if(profit>0){if(TimeCurrent()-time>=PauseMin){OrdHisCount=OrdersHistoryTotal();return(true);}else{return(false);}}
return(false);
}

int start() {
      TakeProfit = TakeProfit1;
//      Kolenomac = iStochastic(NULL,MTS,Kperiod,Dperiod,Slowing,MODE_SMA,0,MODE_MAIN,1);
//      Kolenosig = iStochastic(NULL,MTS,Kperiod,Dperiod,Slowing,MODE_SMA,0,MODE_SIGNAL,1);
   double l_iclose_8;
   double l_iclose_16;
   ...
   for (g_pos_300 = OrdersTotal() - 1; g_pos_300 >= 0; g_pos_300--) {
      ...
      if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_176) {
         if (OrderType() == OP_BUY) {
            gi_320 = TRUE;
            gi_324 = FALSE;
//            Rest = (OrderOpenPrice() - NormalizeDouble(Ask,Digits))/Point;
//            Restlot = OrderLots(); 
            break;
         }
      }
      if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_176) {
         if (OrderType() == OP_SELL) {
            gi_320 = FALSE;
            gi_324 = TRUE;
//            Rest = (NormalizeDouble(Bid,Digits) - OrderOpenPrice())/Point;
//            Restlot = OrderLots(); 
            break;
         }
      }
   ...
   if (gi_304 > 0 && gi_304 <= MaxTrades) {
      ...
      if   (gi_304 > KolTakeProfit){ TakeProfit = TakeProfit2;} 
      if (gi_320 && gd_236 - Ask >= PipStep * Point ) gi_316 = TRUE; // условие колен на бай
      if (gi_324 && Bid - gd_244 >= PipStep * Point ) gi_316 = TRUE; // условие колен на сел
   }
   if (gi_304 < 1) {
      gi_324 = FALSE;
      gi_320 = FALSE;
   ...
   if (gi_316) {
      ...
      if (gi_324) {
         ...
         RefreshRates();
         gi_328 = OpenPendingOrder(1, gd_292, Bid, slip, Ask, Stoploss, 0, gs_bmts_272 + "-" + gi_288, g_magic_176, 0, HotPink);
         if (gi_328 < 0) {
         ...
         if (gi_320) {
            gi_288 = gi_304;
            gd_292 = NormalizeDouble(GetLots() * MathPow(LotExponent, gi_288), lotdecimal);
            gi_328 = OpenPendingOrder(0, gd_292, Ask, slip, Bid, Stoploss, 0, gs_bmts_272 + "-" + gi_288, g_magic_176, 0, Lime);
            if (gi_328 < 0) {
               Print("Error: ", GetLastError());
               return (0);
            }
...
//      Vhodmac = iMACD(NULL,MTF,BystrVhod,MedlVhod,SignVhod,PRICE_CLOSE,MODE_MAIN,1);
//      Vhodsig = iMACD(NULL,MTF,BystrVhod,MedlVhod,SignVhod,PRICE_CLOSE,MODE_SIGNAL,1);
      g_bid_220 = Bid;
      g_ask_228 = Ask;
      if (!gi_324 && !gi_320) {
         gi_288 = gi_304;
         gd_292 = NormalizeDouble(GetLots() * MathPow(LotExponent, gi_288), lotdecimal); // тут лот входа
         if(CloseFriday==true&&DayOfWeek()==5&&TimeCurrent()>=StrToTime(CloseFridayHour+":00"))return(0);
         if (l_iclose_8 > l_iclose_16) { // условие первого ордера на сел
            gi_328 = OpenPendingOrder(1, gd_292, g_bid_220, slip, g_bid_220, Stoploss, 0, gs_bmts_272 + "-" + gi_288, g_magic_176, 0, HotPink);
            if (gi_328 < 0) {
               Print("Error: ", GetLastError());
               return (0);
            }
         ...
         if (l_iclose_8 < l_iclose_16){  // условие первого ордера на бай
            gi_328 = OpenPendingOrder(0, gd_292, g_ask_228, slip, g_ask_228, Stoploss, 0, gs_bmts_272 + "-" + gi_288, g_magic_176, 0, Lime);
            if (gi_328 < 0) {
               Print("Error: ", GetLastError());
               return (0);
            }
