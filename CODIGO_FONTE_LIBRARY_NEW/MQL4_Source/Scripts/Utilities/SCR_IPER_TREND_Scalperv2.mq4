//Available @  https://fxcodebase.com/code/viewtopic.php?f=38&t=76166
...
input bool            RSI_on      = true;              // RSI On?
input ENUM_TIMEFRAMES rsi_tf      = PERIOD_H1;         // RSI Time Frame
...
class ConditionRsiOverbougth
{
  public:
    bool evaluate()
    {
        if (!RSI_on) return true;
        return iRSI(NULL, rsi_tf, rsi_periods, PRICE_CLOSE, 1) > ob_level;
    }
};
...
class ConditionRsiOversold
{
  public:
    ...
    {
        if (!RSI_on) return true;
        return iRSI(NULL, 0, rsi_periods, PRICE_CLOSE, 1) < os_level;
    }
};
...
input string             ma_filter_title   = "== Moving Average FILTER =="; // ————————————————————————
input bool               ma_filter_on      = true;                           // Moving Average Filter On?
input string             ma_title          = "== Moving Average 1 Setup =="; // ————————————————————————
input int                ma1_period        = 25;                             // Periods:
input ENUM_TIMEFRAMES    ma1_tf            = PERIOD_H1;                      // Time Frame:
...
input ENUM_TIMEFRAMES    ma2_tf            = PERIOD_H1;                      // Time Frame:
...
class ConditionEmasTrendUp
{
  public:
    bool evaluate()
    {
        if (!ma_filter_on) return true;
        return ma1(1) > ma2(1);
    }
};
ConditionEmasTrendUp cdEmasTrendUp;

class ConditionEmasTrendDn
{
  public:
    bool evaluate()
    {
...
void OnTick()
{
    ...
    if (CloseTradesAtPercentageDrawdown) {
        ...
        if ((Gd_00000 <= ((Gd_00001 * AccountBalance()) / 100))) {
            ...
            CloseAllExpertOrders(tmp_str00000);
        }
    }
    if (CloseTradesAtFixedDrawdown) {
        ...
        if ((Gd_00001 <= Gd_00003)) {
            ...
            CloseAllExpertOrders(tmp_str00001);
        }
    }
    if (Ib_00078) {
    ...
    if (Ib_00024 != true) {
        ...
        if (Gi_00008 == 0 && Lb_FFFFC) {
            ...
            // sell
            bool rsi_condition = cdRsiOverbougth.evaluate();
            bool emas_condition = cdEmasTrendDn.evaluate();
            if (rsi_condition && emas_condition) 
               // Ii_00040 = OrderSend(_Symbol, 1, Gd_0000E, Bid, 5, 0, 0, Is_00030, Magic, 0, 255);
               Ii_00040 = OrderSend(_Symbol, 1, Gd_0000E, Bid, 5, 0, 0, Is_00030, Magic, 0, Crimson);
        }
        Gi_0000E = -1;
        Gi_00010 = 0;
        ...
        if (Gi_00010 == 0 && Lb_FFFFE) {
            ...
            // Buy
            rsi_condition = cdRsiOversold.evaluate();
            emas_condition = cdEmasTrendUp.evaluate();
            if (rsi_condition && emas_condition) 
            // Ii_00040 = OrderSend(_Symbol, 0, Gd_00016, Ask, 5, 0, 0, Is_00030, Magic, 0, 32768);
            Ii_00040 = OrderSend(_Symbol, 0, Gd_00016, Ask, 5, 0, 0, Is_00030, Magic, 0, RoyalBlue);
        }
        Gi_00016 = -1;
        Gi_00018 = 0;
...
void ApplyTrailingStopToOpenOrd()
{
    int    Li_FFFFC;
    double Ld_FFFF0;
    double Ld_FFFE8;
...
void CloseAllExpertOrders(string Fa_s_00)
{
    string tmp_str00000;
    string tmp_str00001;
    string tmp_str00002;
