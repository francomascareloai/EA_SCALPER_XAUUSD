...
#property copyright "Copyright AI SCAPLER 2019 "
#property description "It's a demo version that can be run only in the Strategy tester. To use the product without limitations purchase the full version"




extern string _="===== Volume ==============================";
extern double LotsSize=0.0;
extern string __="----------------------------------------------";
extern double LotsPer1000=0.3;
extern double MinLots = 0.01;
extern double MaxLots = 60.0;
extern string ___="===== Protection ============================";
extern double def_MaxSpread=1.3; // Maximum spread
extern string _____ = "===== Confirmation ==========================";
extern int Filter=185; //Candle Filter in Point
extern int BB_PARAM = 23; // BB settings
extern int RSI_PARAM = 18; // RSI settings
extern double RSIBuyLevel=50.0; //RSI Buy Level
extern double RSISellLevel=50.0; //RSI Sell Level
extern double def_SL = 3.0;  //Stop loss in pips
extern double Distance_trailing =2.1;//Trailing stop distance in pips
extern double Start_trailing =3.0;//Trailing stop start in pips
extern int                 Slippage     = 3;               // Maximum Price Slippage
extern string ______="===== More ===============================";
extern int MyMagicNumber=68000;
extern string Author_="https://aiscalper.com";

//If you want to use license code restriction
string licenseCode = "1234-ABCD-9876-ZXYW" ;     //ENTER LICENSE CODE HERE
...
double Distance = Distance_trailing *10.0;
double StartTS=Start_trailing *10.0;

// EA Restriction Variables
datetime demoExpiry = D'2020.12.12 00:00';      // This is is format YYYY.MM.DD
string authCode = "1234-ABCD-9876-ZXYW";        // This is check for user input from above. Both must match for EA to operate
...
//+------------------------------------------------------------------+
   ...
   /*if(!IsTesting())
     {
      Comment("It's a demo version that can be run only in the Strategy tester");
      Print("It's a demo version that can be run only in the Strategy tester ");
      Alert("It's a demo version that can be run only in the Strategy tester ");
      ExpertRemove();
      return(0);
     }*/
//+------------------------------------------------------------------+
...
/*
   if(TimeCurrent()>demoExpiry)
     {
      Alert("EA Demo has expired.");
      ExpertRemove();
     }*/

// Account Number Restriction
//if(AccountNumber()!=1000019797)
//{
// Alert("UnAuth. Account - EA Removed");
// ExpertRemove();
//}

// License Code Restriction
/*
   if(licenseCode!=authCode)
     {
      Alert("Invalid License Code - EA Removed");
      ExpertRemove();
     }*/
   ...
   Gd_248Start = NormalizeDouble(StartTS * G_point_196, G_digits_192);
   ...
   if(!IsTesting())
     {
      for(int Li_0=1; Li_0<=Gi_392; Li_0++)
         ObjectDelete("Padding_rect"+Li_0);
...
//  double ima_92 = iMA(NULL, 0, MAPeriod, Gi_180, MAMethod, PRICE_HIGH, 0);
   double ibands_84=iBands(Symbol(),Period(),BB_PARAM,2.0,0,PRICE_CLOSE,MODE_UPPER,0);
   double ibands_92=iBands(Symbol(),Period(),BB_PARAM,2.0,0,PRICE_CLOSE,MODE_LOWER,0);
   ...
   if(Ld_172>Gd_256)
     {
      if(Bid<ibands_92 && irsi_96<RSIBuyLevel)
         Li_180=-1;
      else
         if(Bid>ibands_84 && irsi_96>RSISellLevel)
            Li_180=1;
     }
   int count_184=0;
      ...
      if(OrderSelect(pos_108,SELECT_BY_POS,MODE_TRADES))
        {
         if(OrderMagicNumber()==MyMagicNumber)
           {
            cmd_188=OrderType();
            if(cmd_188==OP_BUYLIMIT || cmd_188==OP_SELLLIMIT)
               continue;
            if(OrderSymbol()==Symbol())
              {
               count_184++;
               switch(cmd_188)
                 {
                  case OP_BUY:
                     ...
                     if((!((Ld_44==0.0 || price_60>Ld_44)))|| !(price_60-OrderOpenPrice()>=Gd_248Start))
                        break;
                  ...
                  case OP_SELL:
                     ...
                     if((!((Ld_44==0.0 || price_60<Ld_44)))|| !(OrderOpenPrice()-price_60>=Gd_248Start))
                        break;
                  ...
                  case OP_BUYSTOP:
                     ...
                     if(!((price_24<Ld_36)))
                        break;
                     price_60= NormalizeDouble(price_24 -(StopLoss+MarketInfo(Symbol(),MODE_SPREAD)) * Point,G_digits_192);
      ...
      if(Li_180<0)
        {
         ...
         if(MarketInfo(Symbol(),MODE_SPREAD)<def_MaxSpread)
            ticket_20=OrderSend(Symbol(),OP_BUYSTOP,Ld_204,price_24,G_slippage_264,price_60,0,Author_,MyMagicNumber,0,Lime);
         if(ticket_20<=0)
           {
            error_8=GetLastError();
      ...
      else
        {
         ...
         price_60 = NormalizeDouble(price_24 + (StopLoss+ MarketInfo(Symbol(),MODE_SPREAD)) * Point, G_digits_192);
         if(MarketInfo(Symbol(),MODE_SPREAD)<def_MaxSpread)
            ticket_20=OrderSend(Symbol(),OP_SELLSTOP,Ld_204,price_24,G_slippage_264,price_60,0,Author_,MyMagicNumber,0,Orange);
         if(ticket_20<=0)
           {
            error_8=GetLastError();
...
void f0_8()
  {
   ...
   for(int Li_0=1; Li_0<=Gi_392; Li_0++)
     {
      if(ObjectFind("Padding_rect"+Li_0)==-1)
         ObjectCreate("Padding_rect"+Li_0,OBJ_RECTANGLE,0,G_time_352,Gd_336 -(Gd_336-Gd_344)/Gi_392 *(Li_0-1),G_datetime_360,Gd_336 -(Gd_336-Gd_344)/Gi_392*Li_0);
...
int f0_9(string As_0,string As_8,int Ai_16,int Ai_20)
  {
   ...
   if(str2int_24 > str2int_36)
      Gi_380 = str2int_24 + (str2int_36 - str2int_24) / Ai_16 * Ai_20;
   if(str2int_24 < str2int_36)
      Gi_380 = str2int_24 - (str2int_24 - str2int_36) / Ai_16 * Ai_20;
   if(str2int_28 > str2int_40)
      Gi_384 = str2int_28 + (str2int_40 - str2int_28) / Ai_16 * Ai_20;
   if(str2int_28 < str2int_40)
      Gi_384 = str2int_28 - (str2int_28 - str2int_40) / Ai_16 * Ai_20;
   if(str2int_32 > str2int_44)
      Gi_388 = str2int_32 + (str2int_44 - str2int_32) / Ai_16 * Ai_20;
   if(str2int_32 < str2int_44)
      Gi_388 = str2int_32 - (str2int_32 - str2int_44) / Ai_16 * Ai_20;
   Gi_384 *= 256;
   Gi_388<<= 16;
...
void f0_7()
  {
   ...
   string text_8="Spread: "+DoubleToStr(Ld_0,1)+" points";
...
int f0_1()
  {
   if(ObjectFind("FiboUp")==-1)
      ObjectCreate("FiboUp",OBJ_FIBO,0,G_datetime_468,G_ihigh_444+Gd_460,G_datetime_468,G_ihigh_444);
   else
     {
   ...
   if(ObjectFind("FiboDn")==-1)
      ObjectCreate("FiboDn",OBJ_FIBO,0,G_datetime_468,G_ilow_452-Gd_460,G_datetime_468,G_ilow_452);
   else
     {
   ...
   if(Gi_440)
     {
      if(ObjectFind("FiboIn")==-1)
         ObjectCreate("FiboIn",OBJ_FIBO,0,G_datetime_468,G_ihigh_444,G_datetime_468+86400,G_ilow_452);
      else
        {
