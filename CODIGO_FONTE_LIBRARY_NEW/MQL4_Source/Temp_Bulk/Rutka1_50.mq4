...
int start(){
   ...
   double MA50H = MathRound(iMA(NULL,0,50,0,MODE_LWMA,PRICE_HIGH,0)/Point)*Point;
   double MA50L = MathRound(iMA(NULL,0,50,0,MODE_LWMA,PRICE_LOW,0)/Point)*Point;
   double Spread = Ask-Bid; // MarketInfo(TradeSymbol,MODE_SPREAD);
   double BuyPrice      =MA50H + Spread+PipDifference*Point;
   double BuyStopLoss   =MA50L - Point;
   double BuyTakeProfit =MA50H +(PipDifference+TakeProfit)*Point;
   double SellPrice     =MA50L -(PipDifference)*Point;
   double SellStopLoss  =MA50H + Spread+Point;
   double SellTakeProfit=MA50L - Spread-(PipDifference+TakeProfit)*Point;
