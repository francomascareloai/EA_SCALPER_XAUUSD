//+------------------------------------------------------------------+
//|                                            EA Phonenix Fixed.mq4 |
//|                                             Copyright 2021, SMV  |
//|                                             https://www.SMV.TECH |
//+------------------------------------------------------------------+

#property copyright "Copyright 2021, SMV TECH"
#property link      "http://www.SMV.TECH"

#property description "\r\n*******SMV TECH*******"
#property description "This is an Automated trading robot design by SMV Team. Please visit the web site for more details."
#property description "WhatsApp: +60 12 311 ****"
#property description "http://www.smv.tech"
#property description "E-mail: info@smv.tech"
#property strict
...
input    int MagicID=11223344;
input    double LOTS=0.01;
input    int risk=20;//risk: 0-->fixed lots
input    double SL=60;
input    double TP=30;
extern   bool Trailing = true;
extern   double TrailingExtra = 0.05;
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
union Price {
...
//+------------------------------------------------------------------+
void OnTick() {
   //if (TimeHour(TimeCurrent()) == 0 || TimeHour(TimeCurrent()) == 4 || TimeHour(TimeCurrent()) == 8 
   //   || TimeHour(TimeCurrent()) == 12 || TimeHour(TimeCurrent()) == 16 || TimeHour(TimeCurrent()) == 20) {
   ...
   double wpr= iWPR(Symbol(),240,4,0);
   double ao = iAO(Symbol(),240,0);
...
//+------------------------------------------------------------------+
   while((bClosed==false) && (nAttemptsLeft>=0)) {
      ...
      if(!ecnBroker)
         bClosed=OrderSend(Symbol(),a_cmd_0,lots_32,price_24,Slippage,price_SL,price_TP,MagicID,0,color_8);
      else
         bClosed=OrderSend(Symbol(),a_cmd_0,lots_32,price_24,Slippage,0,0,MagicID,0,color_8);
      ...
      if(bClosed<=0) {
         ...
         if(nErrResult == ERR_TRADE_CONTEXT_BUSY || nErrResult == ERR_NO_CONNECTION) {
            Sleep(50);
            continue;
         }
      }
      ...
      for(int i=OrdersTotal()-1; i>=0; i--) {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
            if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicID)
               if(OrderType()==OP_BUY)
                  clo=OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),10,clrRed);
