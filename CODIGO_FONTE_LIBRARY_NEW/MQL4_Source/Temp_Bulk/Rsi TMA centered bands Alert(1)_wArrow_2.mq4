...
int init()
{
   HalfLength=MathMax(HalfLength,1);
         ...
         SetIndexLabel(1,"Channel Middle");
         SetIndexBuffer(2,buffer3); 
         SetIndexLabel(2,"Channel Top");
         SetIndexBuffer(3,buffer4);
         SetIndexLabel(3,"Channel Bottom");
         SetIndexBuffer(4,buffer5);
         SetIndexLabel(4,"Arrow Up");
         SetIndexStyle(4,DRAW_ARROW);
         SetIndexArrow(4,233);
         SetIndexBuffer(5,buffer6);
         SetIndexLabel(5,"Arrow Down");
...
int start()
{
   ...
   for (i=limit; i>=0; i--)
   {
      ...
      if(buffer1[i]>buffer3[i]&&buffer1[i+1]<=buffer3[i+1]){buffer6[i]=buffer1[i];buffer5[i]=EMPTY_VALUE;}
      if(buffer1[i]<buffer4[i]&&buffer1[i+1]>=buffer4[i+1]){buffer5[i]=buffer1[i];buffer6[i]=EMPTY_VALUE;}
   }
