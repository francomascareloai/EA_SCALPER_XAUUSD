...
int start()
  {
   ...
   double rsi1=iRSI(Symbol(),PERIOD_M1,RSIperiod,PRICE_CLOSE,1);
   double rsi2=iRSI(Symbol(),PERIOD_M5,RSIperiod,PRICE_CLOSE,0);
   double rsi3=iRSI(Symbol(),PERIOD_M5,RSIperiod,PRICE_CLOSE,1);
// double HTb=iCustom(Symbol(),0,"HalfTrend-1.02",0,0); //buy
// double HTs=iCustom(Symbol(),0,"HalfTrend-1.02",1,0); //buy
//--- open position
   if(OpenSELL && OrderSell<1 && rsi<SellLevel && rsi2<SellLevel && rsi1>SellLevel && rsi3>SellLevel) OPSELL();
   if(OpenBUY && OrderBuy<1 && rsi>BuyLevel && rsi2>BuyLevel && rsi1<BuyLevel && rsi3>BuyLevel) OPBUY();
//--- close position by signal
   if(CloseBySignal)
     {
      if(OrderBuy>0 && rsi<SellLevel && rsi2<SellLevel && rsi1>SellLevel && rsi3>SellLevel) CloseBuy();
      if(OrderSell>0 && rsi>BuyLevel && rsi2>BuyLevel && rsi1<BuyLevel && rsi3>BuyLevel) CloseSell();
     }
//---
   return(0);
  }
