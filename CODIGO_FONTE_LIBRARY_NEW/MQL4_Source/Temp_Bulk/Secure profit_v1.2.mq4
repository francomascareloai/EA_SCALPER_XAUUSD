...
// exported variables
extern string comm       = "Открытие-закрытие (пн.-пт.):";
extern int MondayStartHour = 6;
extern int FridayLastTradeHour   = 18;
extern string comm1      = "Автоматический расчет лота (вкл/откл):";
extern bool AutoMM       = FALSE;
extern double  MaxRisk   = 10;
extern double Max_lot    = 10;
extern string comm2      = "Постоянный лот:";
extern double Lot = 0.1;
extern string comm3      = "Параметры:";
...
int start()
{
    ...
    if (DayOfWeek() == 5 && Hour() > FridayLastTradeHour) return (0);
    
    if (AutoMM) Lot=AvtoLot(MaxRisk);
...
//+--------------------------------------------------------------------------------------------------------------+
double AvtoLot(int Risk){
  double Free    =AccountFreeMargin();
  double One_Lot =MarketInfo(Symbol(),MODE_MARGINREQUIRED);
  double Min_Lot =MarketInfo(Symbol(),MODE_MINLOT);
  double Max_Lot =MarketInfo(Symbol(),MODE_MAXLOT);
if (Max_lot>0&&Max_Lot>Max_lot)Max_Lot=Max_lot;
