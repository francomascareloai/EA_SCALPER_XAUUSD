//+------------------------------------------------------------------+
//|                                                  MEILING FUN.mq4 |
//|                        Copyright 2077, Briella Audrey.           |
//|                                  https://t.me/simpleforextools   |
//+------------------------------------------------------------------+
#property copyright "Copyright 2007, TANPA MIKIR."
...
input bool zone_show_proven = true;          // Show Proven Zones
...
// check for entries
   for(int i=0; i<zone_count; i++)
     {
      // Skip zones that are not displayed based on user settings
      if(!IsZoneVisible(i))
         continue;
...
//+------------------------------------------------------------------+
//| New function to check if zone should be visible                    |
//+------------------------------------------------------------------+
bool IsZoneVisible(int index)
  {
   if(zone_strength[index]==ZONE_WEAK && !zone_show_weak)
      return(false);
   if(zone_strength[index]==ZONE_UNTESTED && !zone_show_untested)
      return(false);
   if(zone_strength[index]==ZONE_TURNCOAT && !zone_show_turncoat)
      return(false);
   if(zone_strength[index]==ZONE_VERIFIED && !zone_show_verified)
      return(false);
   if(zone_strength[index]==ZONE_PROVEN && !zone_show_proven)
      return(false);
   return(true);
  }
//+------------------------------------------------------------------+
