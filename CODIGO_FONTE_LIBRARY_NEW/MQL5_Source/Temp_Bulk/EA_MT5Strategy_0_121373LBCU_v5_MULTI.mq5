//+------------------------------------------------------------------+
//| Strategy 0.121373 EA
//|
//|    Generated by StrategyQuant X Build 138 for MetaTrader5 (hedged)
//|    Generated at 12/10/2024 02:03
//| 
//|    Backtested on XAUUSDGOLD / H1, 2020.10.01 - 2024.10.16 
//+------------------------------------------------------------------+
...
input string CustomComment = "MT5Strategy_0_121373LBCU";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 98623;        //MagicNumber
...
input int StochSlowDKPeriod1 = 5;        //StochSlowDKPeriod1
input int StochSlowDDPeriod1 = 3;        //StochSlowDDPeriod1
input int StochSlowDSlowing1 = 3;        //StochSlowDSlowing1
input double PriceEntryMult1 = 1.4;        //PriceEntryMult1
input double SL2BEAddPipsCoef1 = 5;        //SL2BEAddPipsCoef1
input double StopLossCoef1 = 2.6;        //StopLossCoef1
input double TrailingStop1 = 159;        //TrailingStop1
input int BBWidthRatioPeriod1 = 14;        //BBWidthRatioPeriod1
...
input bool LimitMaxDistanceFromMarketPrice = true;   //Limit max distance
input double MaxDistanceFromMarketPct = 5;   //Max distance %
...
#define STOCHASTIC_1 0     //iCustom(NULL,0, "SqStochastic", StochSlowDKPeriod1, StochSlowDDPeriod1, StochSlowDSlowing1, MODE_SMA, STO_LOWHIGH)
#define BBWIDTHRATIO_1 1     //iCustom(NULL,0, "SqBBWidthRatio", BBWidthRatioPeriod1, 7.0, PRICE_HIGH)
#define ATR_1 2     //iCustom(NULL, 0, "SqATR", 47)
#define ATR_2 3     //iCustom(NULL, 0, "SqATR", 25)
#define BBWIDTHRATIO_2 4     //iCustom(NULL,0, "SqBBWidthRatio", BBWidthRatioPeriod1, 7, PRICE_LOW)
...
string sqStrategyName = "Strategy 0.121373";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = sqIsRising(STOCHASTIC_1, 2, false, 1, 1);

     ShortEntrySignal = sqIsFalling(STOCHASTIC_1, 2, false, 1, 1);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqLow(NULL,0, 1) + (PriceEntryMult1 * sqGetIndicatorValue(BBWIDTHRATIO_1, 0, 1))), "Current");
      ...
      if(_ticket > 0) {
         ...
         sqSetOrderExpiration(_ticket, 39);

         //Check StopLoss & ProfitTarget
         //sqCheckSLPT(_ticket, sl, pt);
         // MoveSL2BE initialization
         sqSetMoveSL2BE(_ticket, "sqOpen(NULL,0, 1)", SLPTTYPE_LEVEL);
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqHigh(NULL,0, 1) - (PriceEntryMult1 * sqGetIndicatorValue(BBWIDTHRATIO_2, 0, 1))), "Current");
...
bool initIndicators(){
   ...
   indicatorHandles[STOCHASTIC_1] = iCustom(NULL,0, "SqStochastic", StochSlowDKPeriod1, StochSlowDDPeriod1, StochSlowDSlowing1, MODE_SMA, STO_LOWHIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BBWIDTHRATIO_1] = iCustom(NULL,0, "SqBBWidthRatio", BBWidthRatioPeriod1, 7.0, PRICE_HIGH);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(NULL, 0, "SqATR", 47);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_2] = iCustom(NULL, 0, "SqATR", 25);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[BBWIDTHRATIO_2] = iCustom(NULL,0, "SqBBWidthRatio", BBWidthRatioPeriod1, 7, PRICE_LOW);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("sqOpen(NULL,0, 1)")) {
      return (sqOpen(NULL,0, 1));
   }
