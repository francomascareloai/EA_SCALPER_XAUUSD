...
//| XAUUSD-M30 EA
//|
//|    Generated by StrategyQuant X Build 141 for MetaTrader5 (hedged)
//|    Generated at 01/29/2025 14:53
//| 
//|    Backtested on XAUUSDGOLD / M30, 2022.10.09 - 2024.10.08 
//+------------------------------------------------------------------+
...
input string CustomComment = "XAUUSD_M30";
bool IndicatorLoadedWithoutError = true;
input int MagicNumber = 303045;        //MagicNumber
...
input int MomPeriod1 = 20;        //MomPeriod1
input int MomShift1 = 1;        //MomShift1
input bool BooleanValue1 = false;        //BooleanValue1
input double PriceEntryMult1 = 2.9;        //PriceEntryMult1
input int BarsValid1 = 38;        //BarsValid1
input bool ReplaceExisting1 = true;        //ReplaceExisting1
input bool AllowDuplicateTrd1 = false;        //AllowDuplicateTrd1
input int ExitAfterBars1 = 1;        //ExitAfterBars1
input double MoveSL2BE1 = 405;        //MoveSL2BE1
input double SL2BEAddPipsCoef1 = 3.7;        //SL2BEAddPipsCoef1
input int SL2BEAddPipsAtrPrd1 = 32;        //SL2BEAddPipsAtrPrd1
input double StopLossCoef1 = 2;        //StopLossCoef1
input int StopLossAtrPeriod1 = 20;        //StopLossAtrPeriod1
input double TrailingStopCoef1 = 3;        //TrailingStopCoef1
input int TrailingStopAtrPrd1 = 82;        //TrailingStopAtrPrd1
input int Shift1 = 2;        //Shift1
input int MTATRPeriod1 = 86;        //MTATRPeriod1
input int MTATRShift1 = 2;        //MTATRShift1
...
input string FridayCloseTime = "22:30";
input string SundayOpenTime = "01:00";
...
input string FridayExitTime = "22:30";
...
input double MaxDistanceFromMarketPct = 3;   //Max distance %
...
#define MOMENTUM_1 0     //iMomentum(NULL,0, MomPeriod1, PRICE_CLOSE)
#define MTATR_1 1     //iATR(NULL,0, MTATRPeriod1)
#define ATR_1 2     //iCustom(NULL, 0, "SqATR", SL2BEAddPipsAtrPrd1)
#define ATR_2 3     //iCustom(NULL, 0, "SqATR", StopLossAtrPeriod1)
#define ATR_3 4     //iCustom(NULL, 0, "SqATR", TrailingStopAtrPrd1)
...
string sqStrategyName = "XAUUSD-M30";
...
if (_sqIsBarOpen == true) {
     ...
     LongEntrySignal = sqIsRising(MOMENTUM_1, 2, false, MomShift1);

     ShortEntrySignal = BooleanValue1;

     LongExitSignal = BooleanValue1;

     ShortExitSignal = BooleanValue1;
...
//------------------------
...
)
   {
      // Action #1
        // Enter at Stop
      openPrice = sqFixMarketPrice((sqHigh(NULL,0, Shift1) + (PriceEntryMult1 * sqGetIndicatorValue(MTATR_1, 0, MTATRShift1))), "Current");
      ...
      size = sqMMRiskFixedAccountPct("Current",ORDER_TYPE_BUY_STOP,openPrice,sl,mmRiskPercent,mmDecimals,mmStopLossPips,mmLotsIfNoMM,mmMaxLots,mmMultiplier,mmStep);
      ...
      _ticket = openPosition(
         ...
         ReplaceExisting1, // Replace existing order (if it exists)
         AllowDuplicateTrd1  // Allow duplicate trades
      );

      if(_ticket > 0) {
         // order was successfuly placed, set or initialize all its exit methods (SL, PT, Trailing Stop, Exit After Bars, etc.)
         sqSetOrderExpiration(_ticket, BarsValid1);
         ...
         sqSetSL2BEAddPips(_ticket, "SL2BEAddPipsCoef1 * sqGetIndicatorValue(ATR_1, 1)");
...
bool initIndicators(){
   ...
   indicatorHandles[MOMENTUM_1] = iMomentum(NULL,0, MomPeriod1, PRICE_CLOSE);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[MTATR_1] = iATR(NULL,0, MTATRPeriod1);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_1] = iCustom(NULL, 0, "SqATR", SL2BEAddPipsAtrPrd1);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_2] = iCustom(NULL, 0, "SqATR", StopLossAtrPeriod1);
   
   ArrayResize(indicatorHandles, ArraySize(indicatorHandles) + 1, 10);
   indicatorHandles[ATR_3] = iCustom(NULL, 0, "SqATR", TrailingStopAtrPrd1);
...
double sqGetValueByIdentification(int idHash) {
   ...
   if(idHash == sqStringHash("SL2BEAddPipsCoef1 * sqGetIndicatorValue(ATR_1, 1)")) {
      return (SL2BEAddPipsCoef1 * sqGetIndicatorValue(ATR_1, 1));
   }
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/ClosedPLInMoneyTime.mq5 -------------------------------------+

double sqGetClosedPLInMoneyTime(string symbol, int magicNum,int hour) {
   ...
   structTime.sec  = 0;
   structTime.hour = 1;
   datetime timeFrom  = StructToTime( structTime );
   ...
   for(int i=HistoryDealsTotal()-1; i>=0; i--) {
      ...
      if(magicNum != 0) {                            
      if(!checkMagicNumber(dealMagicNumber) || dealMagicNumber != magicNum)  continue;
      }
      if(symbol != "Any" && dealSymbol != correctSymbol(symbol)) continue;
      if(type == 0) continue;
   
      datetime OrderCloseTime = (datetime)HistoryOrderGetInteger(ticket, ORDER_TIME_DONE);

     
      if (OrderCloseTime>timeFrom){
                                               
         //printf("Pos Close Time: "+(datetime)HistoryOrderGetInteger(ticket, ORDER_TIME_DONE)+"  timeFrom: "+timeFrom+" type: "+type+" OrderCloseTime>timeFrom)"+(OrderCloseTime>timeFrom));
			sumPL = sumPL+HistoryDealGetDouble(ticket, DEAL_PROFIT);
			} 
	  

		if (timeFrom > OrderCloseTime){
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/CrossesAboveAdaptive.mq5 -------------------------------------+

bool sqCrossesAboveAdaptive(string indicatorIdentificationLeft,string indicatorIdentificationRight,int TradesLookBack, int ExitAfterBars,int Methodology,double Treshold) {

   int calculatedBars = Bars(_Symbol, _Period); // iBars replaced with Bars in MQL5
   int historyOrdersCount = HistoryDealsTotal(); // OrdersHistoryTotal replaced with HistoryDealsTotal
   
   double val11 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft,1), 5);
   double val21 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight,1), 5);

   double val10 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft,0), 5);
   double val20 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight,0), 5);
   
   bool mainCondition = false;
   bool adaptiveCondition = false;
   ...
   if(val11 < val21 && val10 > val20) {
      ...
      for(int i = ExitAfterBars; i < calculatedBars - 1; i++) {
         double pval11 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft,i+1), 5);
         double pval21 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight,i+1), 5);
         double pval10 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft,i), 5);
         double pval20 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight,i), 5);
         
         // INNER CONDITION CHECK
         if(pval11 < pval21 && pval10 > pval20) {

            count++;
            double curPrice = iOpen(_Symbol, _Period, i);
            double futPrice = iOpen(_Symbol, _Period, i - ExitAfterBars);
            
            // Calculation of profit factor
            if((futPrice - curPrice) > 0) {
               pfPos += MathAbs((futPrice - curPrice));
            } else {
               pfNeg += MathAbs((futPrice - curPrice));
            }

            // Percentage performance per signal
            absBarsPnL = ((futPrice - curPrice) / curPrice) * 100;

            if(absBarsPnL > 0) profTrades++;

            avgPerf += (MathAbs(absBarsPnL) / ExitAfterBars);
            ...
            if(Methodology == 4) {
               ...
               for(int b = i; b > i - ExitAfterBars; b--) {
                  double curOpen = MathLog(iOpen(NULL, 0, b));
                  double futOpen = MathLog(iOpen(NULL, 0, b - 1));
                  
                  double barRet = ((futOpen - curOpen) / curOpen) * 100;
                  
                  if(barRet > 0) posRes += MathAbs(barRet);
                  else negRes += MathAbs(barRet);
               }
               ipfPos += posRes;
               ipfNeg= negRes;
            }           
            if(count == TradesLookBack) break;
         }
      }

      //if(count != 0) intPF = (intPF / (double)count);


      if (ipfNeg!=0) intPF = (ipfPos/(double)ipfNeg);
		else intPF =3;
      
      // Average bars absolute performance per trade
      avgPerf = avgPerf / TradesLookBack;

      // Profit factor per signal
      if(pfNeg != 0) profFac = pfPos / pfNeg;
      
      // % Win per signal
      if(count != 0) winPerc = (profTrades / (double)count) * 100;
   }
   ...
   else if (Methodology == 4){
      metric = intPF;
   }

   if(metric > Treshold) adaptiveCondition = true;
   else adaptiveCondition = false;

   return (mainCondition && adaptiveCondition);
}

//+----------------------------- Include from /MetaTrader5/CustomFunctions/CrossesBelowAdaptive.mq5 -------------------------------------+

bool sqCrossesBelowAdaptive(string indicatorIdentificationLeft,string indicatorIdentificationRight,int TradesLookBack, int ExitAfterBars,int Methodology,double Treshold) {
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/IsGreaterAdaptive.mq5 -------------------------------------+

bool sqIsGreaterAdaptive(string indicatorIdentificationLeft, 
                        string indicatorIdentificationRight,
                        int TradesLookBack,
                        int Methodology,
                        double Treshold) 
{
    int calculatedBars = Bars(_Symbol, PERIOD_CURRENT);
    
    // Current and previous values for main condition
    double curVal1 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft, 0), 6);
    double curVal2 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight, 0), 6);
    double prevVal1 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft, 1), 6);
    double prevVal2 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight, 1), 6);
    ...
    // Check main crossing condition
    if(curVal1 > curVal2 && prevVal1 < prevVal2) {
        mainCondition = true;
        int count = 0;
        
        // Historical analysis loop
        for(int i = 0; i < calculatedBars-1; i++) {
            double histVal1 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft, i), 6);
            double histVal2 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight, i), 6);
            double prevHistVal1 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationLeft, i+1), 6);
            double prevHistVal2 = NormalizeDouble(sqGetExpressionByIdentification(indicatorIdentificationRight, i+1), 6);
            
            // Check for exit signals
            if(histVal1 < histVal2 && prevHistVal1 > prevHistVal2) {
                // Calculate performance if we have an entry
                if(entryPrice != 0) {
                    if((exitPrice - entryPrice) > 0) {
                        posPF += MathAbs((exitPrice - entryPrice));
                    } else {
                        negPF += MathAbs((exitPrice - entryPrice));
                    }
                ...
                exitPrice = iOpen(_Symbol, PERIOD_CURRENT, i);
            }
            
            // Check for entry signals
            if(histVal1 > histVal2 && prevHistVal1 < prevHistVal2) {
                // Only record entry if it's not the current signal
                if(iOpen(_Symbol, PERIOD_CURRENT, i) != iOpen(_Symbol, PERIOD_CURRENT, 0)) {
                    entryPrice = iOpen(_Symbol, PERIOD_CURRENT, i);
                }
            }
            
            // Break if we've analyzed enough trades
            if(count == TradesLookBack) break;
        }
        
        // Calculate final metrics
        if(negPF > 0) avgPF = posPF/negPF;
    }
    
    // Determine final metric based on methodology
    double metric = 0;
    if(Methodology == 1) {
        metric = avgPF;
    } else if(Methodology == 2) {
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/IsLowerAdaptive.mq5 -------------------------------------+

bool sqIsLowerAdaptive(string indicatorIdentificationLeft, 
                        string indicatorIdentificationRight,
                        int TradesLookBack,
                        int Methodology,
                        double Treshold) 
        ...
        for(int i = 0; i < calculatedBars-1; i++) {
            ...
            if(histVal1 > histVal2 && prevHistVal1 < prevHistVal2) {
                ...
                if(entryPrice != 0) {
                    ...
                    double PnL = ((entryPrice-exitPrice)/entryPrice)*100;
                    perf += PnL;
                    count++;
                }
                exitPrice = iOpen(_Symbol, PERIOD_CURRENT, i);
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/StrictlyGreaterCount.mq5 -------------------------------------+


//+------------------------------------------------------------------+


bool sqIsStrictlyGreaterCount(string indicatorIdentificationLeft,string indicatorIdentificationRight, int bars,bool NotStrict,int shift) {
...
//+----------------------------- Include from /MetaTrader5/CustomFunctions/StrictlyLowerCount.mq5 -------------------------------------+


//+------------------------------------------------------------------+

bool sqIsStrictlyLowerCount(string indicatorIdentificationLeft,string indicatorIdentificationRight, int bars,bool NotStrict,int shift) {
