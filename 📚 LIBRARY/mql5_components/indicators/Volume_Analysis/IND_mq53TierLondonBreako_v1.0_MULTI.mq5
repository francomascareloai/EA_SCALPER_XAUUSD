...
input string Info         = VERSION; // version number information
input string StartTime    = "06:00";    // time for start of price establishment window
input string EndTime      = "09:14";    // time for end of price establishment window
input string SessionEndTime= "04:30";   // end of daily session; tomorrow is another day!
input color  SessionColor = Linen; // show Session periods with a different background color
input int    NumDays      = 200;         // days back
input int    MinBoxSizeInPips = 15;     // min tradable box size; when box is smaller than that, you should at least reduce your usual lot-size if you decide to trade it;
input int    MaxBoxSizeInPips = 80;     // max tradable box size; don't trade when box is larger than that value
input bool   LimitBoxToMaxSize = true; // when true, a box larger than MaxBoxSizeInPips will be limited to MaxBoxSizeInPips, and centered on the EMA(box_time_range) value.
input bool   StickBoxToLatestExtreme = false;  // (applies when "LimitBoxToMaxSize" is true) when true, the box will "stick" to the box high or low, whichever comes last; else it will be centered on the EMA(box_time_range) value;
input bool   StickBoxOusideSRlevels = false;  // when true, we'll use the latest highest/lowest PA as S/R level, and "stick" the box to outside of it;
input double TP1Factor    = 1.000;
...
input double TP5Factor    = 4.236;// TP4 and TP5 targets are OPTIONAL: set TP5Factor=0 to allow only up to TP3 target;
...
input color  BoxColorOK   = LightBlue;
input color  BoxColorNOK  = Red;
input color  BoxColorMAX  = Orange;
input color  LevelColor   = Black;
input int    FibLength    = 14;
input bool   showProfitZone = true;
input color  ProfitColor  = LightGreen;
input bool   MondayFix    = True;

input string objPrefix    = "LB2-";  // all objects drawn by this indicator will be prefixed with this
