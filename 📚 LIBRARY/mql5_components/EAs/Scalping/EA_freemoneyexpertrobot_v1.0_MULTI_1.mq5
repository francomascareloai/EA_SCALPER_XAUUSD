...
void OnTick()
{
   ...
   if (SL_Points > 0)
      sl = (direction == 1) ? price - SL_Points * _Point : price + SL_Points * _Point;
   if (TP_Points > 0)
      tp = (direction == 1) ? price + TP_Points * _Point : price - TP_Points * _Point;

   bool result = false;
   if (direction == 1)
      result = trade.Buy(currentLot, _Symbol, price, sl, tp, "Buy");
   else
      result = trade.Sell(currentLot, _Symbol, price, sl, tp, "Sell");

   if (result)
   {
      string dirStr = (direction == 1 ? "BUY" : "SELL");
      string msg = StringFormat("Order Sent: %s\nLot: %.2f\nTP: %.5f\nSL: %.5f", dirStr, currentLot, tp, sl);
