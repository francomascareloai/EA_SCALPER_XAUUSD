...
#property indicator_color1 Lime //wick
#property indicator_color2 Red   //wick
#property indicator_color3 Lime //candle
...
//---- stoch settings
extern int   CCI_Period1     = 14;
extern int   CCI_Period2     = 80;
 int   CCI_Price      = 5;
...
int init()
  {
//---- indicators
   IndicatorShortName("CCI Candles:("+   CCI_Period1 + ", " + CCI_Period2 + ")");
   IndicatorBuffers(4);
...
//+------------------------------------------------------------------+
double CCI     (int i=0, int period = 14)   {return(iCCI(NULL, 0, period, CCI_Price, i));}
//+------------------------------------------------------------------+
...
int start()
  {
   for(int i=MathMax(Bars-1-IndicatorCounted(),1); i>=0; i--)
     {
      double   cci1  =CCI(i, CCI_Period1);
      double   cci2  =CCI(i, CCI_Period2);
      
      if(cci1 > Level1 && cci2 > Level2 )      SetCandleColor(1,i);
      if(cci1 < -Level1 && cci2 < -Level2 )      SetCandleColor(2,i);
