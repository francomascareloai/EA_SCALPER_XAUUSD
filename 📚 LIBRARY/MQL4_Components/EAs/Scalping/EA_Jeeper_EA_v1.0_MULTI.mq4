//+------------------------------------------------------------------+
//|Пробой уровня прошлой свечи            BreakdownLevelCandleMA.mq4 |
...
int           STOPLEVEL,magic=1,tip,TimeBarBay,TimeBarSell,LastDay;
string txt;
//--------------------------------------------------------------------
int init()
{
...
int start()
{
   ...
   double TrPr,StLo;
   MaxPrice=NormalizeDouble(iOpen(NULL,TimeCandle,1)+Delta*Point,Digits);
   MinPrice=NormalizeDouble(iClose(NULL,TimeCandle,1)-Delta*Point,Digits);
   if (risk!=0) Lot = LOT(); 
   if (bay<MaxOrders && TimeBarBay!=iTime(NULL,TimeCandle,0) && BUYOK && Ask>=MinPrice)
   {
      ...
      if (!OrderSend(Symbol(),OP_BUY,Lot,NormalizeDouble(Ask,Digits),3,StLo,TrPr,"BreakdownLevelCandleMA",magic,0,Blue))
         Print("Error BUYSTOP ",GetLastError(),"   ",Symbol(),"   Lot ",Lot,"   Price ",MinPrice,"   SL ",StLo,"   TP ",TrPr);
      else TimeBarBay=iTime(NULL,TimeCandle,0);
   }
   if (sel<MaxOrders && TimeBarSell!=iTime(NULL,TimeCandle,0) && SELLOK && Bid<=MaxPrice)
   {
      ...
      if (!OrderSend(Symbol(),OP_SELL,Lot,NormalizeDouble(Bid,Digits),3,StLo,TrPr,"BreakdownLevelCandleMA",magic,0,Red ))
         Print("Error SELLSTOP ",GetLastError(),"   ",Symbol(),"   Lot ",Lot,"   Price ",MaxPrice,"   SL ",StLo,"   TP ",TrPr);
