//+------------------------------------------------------------------+
//|                                                 MultiLot Sclaper |
//|              Perky Mods      Copyright © 2005, Alejandro Galindo |
//|                                            PerkyAintNoTurkey.com |
//+------------------------------------------------------------------+

// Version changes............ GBP 30 min is best 
//V1 has psar external variables included so they can be changed - PsarStep0.02,PsarMax0.2 are presets
// 
...
//---- 
   ...
   if (OpenOrders<1) 
   {
	 ...
	 // if (iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,0)>iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,1)) { myOrderType=2; }//BUY
	  //if (iMACD(14,26,9,MODE_MAIN,0)<0 and iMACD(14,26,9,MODE_MAIN,0)<iMACD(14,26,9,MODE_MAIN,1)) then OrderType=1;
	 // if (iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,0)<iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,1)) { myOrderType=1; }//SELL
	   if (iSAR(NULL,0,PsarStep,PsarMax,1) <= Low[1]  && iSAR(NULL,0,PsarStep,PsarMax,0) > High[0]) { myOrderType=1; }//SELL
	   if (iSAR(NULL,0,PsarStep,PsarMax,1) >= High[1] && iSAR(NULL,0,PsarStep,PsarMax,0) < Low[0] ) { myOrderType=2; }//BUY
	    // if (iSAR(NULL,0,0.02,0.2,1) <= Low[3]  && MathAbs(iSAR(NULL,0,0.02,0.2,1)-iSAR(NULL,0,0.02,0.2,0))>0.0010) { myOrderType=1; }//SELL
	   	 //  if (iSAR(NULL,0,0.02,0.2,1) >= High[3] && MathAbs(iSAR(NULL,0,0.02,0.2,1)-iSAR(NULL,0,0.02,0.2,0))>0.0010) { myOrderType=2; }//BUY
	// Print (MathAbs(iSAR(NULL,0,0.02,0.2,1)-iSAR(NULL,0,0.02,0.2,0)));
   ...
   {	    
      ...
      if (!IsTesting()) 
      {
	     ...
	     Comment(iSAR(NULL,0,0.002,0.2,1)- iSAR(NULL,0,0.002,0.2,0)," LastPrice=",LastPrice," Previous open orders=",PreviousOpenOrders,"\nContinue opening=",ContinueOpening," OrderType=",myOrderType,"\n",text2,"\nLots=",lotsi,"\n",text);
      }
      ...
      {	
	     ...
	     {		
		  ...
		  //  else { sl=SellPrice+InitialStop*Point;  }
		    // else { sl=(iSAR(NULL,0,PsarStep,PsarMax,0)+5*Point);  }
		    if (OpenOrders!=0) 
		    {
			      mylotsi=lotsi;			
		  ...
		  //if (MathAbs(iSAR(NULL,0,PsarStep,PsarMax,1)-iSAR(NULL,0,PsarStep,PsarMax,0))>0.0005 && DayOfWeek()!=1)
		    {
      ...
      if (myOrderType==2 && ContinueOpening) 
      {
	     ...
	     {		
		   ...
		   // else { sl=BuyPrice-InitialStop*Point; }
		    else { sl=(iSAR(NULL,0,0.02,0.2,0)-5*Point); }
		    if (OpenOrders!=0) {
			   mylotsi=lotsi;			
			   for(cnt=1;cnt<=OpenOrders;cnt++)
			   {
		    ...
		    if (mylotsi>100) { mylotsi=100; }
		    if (MathAbs(iSAR(NULL,0,0.02,0.2,1)-iSAR(NULL,0,0.02,0.2,0))>0.0005 && DayOfWeek()!=1)
		    {
