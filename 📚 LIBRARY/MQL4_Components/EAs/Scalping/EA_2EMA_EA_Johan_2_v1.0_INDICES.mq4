...
double FloatingLossThreshold = 70.00, FloatingProfitThreshold = 30.00;
int ProfitToStartTrailingSL = 25500, TrailingSLValue = 10000;
...
void OnTick() {
   ...
   if(newtime != Time[0]) {
      ...
      double ema200 = iMA(_Symbol, EMA_TIMEFRAME, 200, 0, MODE_EMA, PRICE_CLOSE, 1);
      double ema200Prev = iMA(_Symbol, EMA_TIMEFRAME, 200, 0, MODE_EMA, PRICE_CLOSE, 2);
      ...
      bool twoBearCandles = iClose(_Symbol, _Period, 1) < iOpen(_Symbol, _Period, 1) && 
                           MathAbs(iClose(_Symbol, _Period, 1) - iOpen(_Symbol, _Period, 1)) >= 20 * _Point &&
                           iClose(_Symbol, _Period, 2) < iOpen(_Symbol, _Period, 2) && 
                           MathAbs(iClose(_Symbol, _Period, 2) - iOpen(_Symbol, _Period, 2)) >= 20 * _Point;
      bool twoBullCandles = iClose(_Symbol, _Period, 1) > iOpen(_Symbol, _Period, 1) && 
                           MathAbs(iClose(_Symbol, _Period, 1) - iOpen(_Symbol, _Period, 1)) >= 20 * _Point &&
                           iClose(_Symbol, _Period, 2) > iOpen(_Symbol, _Period, 2) && 
                           MathAbs(iClose(_Symbol, _Period, 2) - iOpen(_Symbol, _Period, 2)) >= 20 * _Point;

      bool isDowntrend = ema200 < ema200Prev;
      bool isUptrend = ema200 > ema200Prev;

      if(MaxTrades == 0 || orders_count < MaxTrades) {
         if(emaFast < emaSlow && emaGap > spreadDistance * distanceMultiplier && 
            twoBearCandles && (orders_count == 0 || enoughDistanceForNewSell) && isDowntrend) {
            ...
            if(t > 0) { lastSellTicket = t; UpdateStopLossValues(); }
                  ...
                  twoBullCandles && (orders_count == 0 || enoughDistanceForNewBuy) && isUptrend) {
...
void UpdateStopLossValues() {
   if(lastSellTicket > 0 && OrderSelect(lastSellTicket, SELECT_BY_TICKET)) {
      hiddenSellStopLoss = iHigh(_Symbol, _Period, 1);
      for(int i = OrdersTotal() - 1; i >= 0; i--) {
   ...
   if(lastBuyTicket > 0 && OrderSelect(lastBuyTicket, SELECT_BY_TICKET)) {
      hiddenBuyStopLoss = iLow(_Symbol, _Period, 1);
      for(int i = OrdersTotal() - 1; i >= 0; i--) {
         if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && 
            OrderMagicNumber() == Magic && OrderSymbol() == Symbol() && OrderType() == OP_BUY) {
            double orderSL = iLow(_Symbol, _Period, 1);
            if(orderSL < hiddenBuyStopLoss || hiddenBuyStopLoss == 0) hiddenBuyStopLoss = orderSL;
         }
      }
   }
}
         ...
         if(OrderType() == OP_BUY) {
            ...
            if(profit >= ProfitToStartTrailingSL) {
               ...
               if(OrderStopLoss() == 0 || newSL > OrderStopLoss()) {
                  bool success = OrderModify(OrderTicket(), OrderOpenPrice(), newSL, OrderTakeProfit(), 0, clrNONE);
