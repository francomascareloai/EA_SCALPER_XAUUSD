...
extern int        Magic             =69;
extern string     comment           ="m icwr.17a";
...
//---bullish (uptrend) fibo
   if(a.high.shift<=a.low.shift && !IsTesting())  {
...
//---bearish (downtrend) fibo
   if(a.low.shift<a.high.shift && !IsTesting())  {
...
//---buy/sell order parameters, close buy/sell parameters
   if(b==0 && rsi>30 && adx.plus.di>adx.minus.di)  {
   ...
   if(s==0 && rsi<70 && rsi>0 && adx.minus.di>adx.plus.di)  {
...
//if stoploss is equal to, or greater than minimum wave range, set bsl to minimum wave range 
      if(OrderStopLoss()<OrderOpenPrice() &&
         OrderOpenPrice()-OrderStopLoss()>=x) {bsl=x;}
//determine if stoploss should be modified
      if(Bid>(OrderOpenPrice()+bsl) &&
         OrderStopLoss()<OrderOpenPrice()+((Bid-(OrderOpenPrice()+bsl))/2))   {
         b.tsl=NormalizeDouble(OrderOpenPrice()+((Bid-(OrderOpenPrice()+bsl))/2),Digits);
         OrderModify(b.ticket,OrderOpenPrice(),b.tsl,OrderTakeProfit(),
                     OrderExpiration(),MediumSpringGreen);}}
//--Short TSL calc...
   double ssl=0;double s.tsl=0;
   if(s.ticket>0) {
      OrderSelect(s.ticket,SELECT_BY_TICKET);
...
//if stoploss is equal to, or greater than minimum wave range, set bsl to minimum wave range
      if(OrderStopLoss()>OrderOpenPrice() &&
         OrderStopLoss()-OrderOpenPrice()>=x)   {ssl=x;}
//determine if stoploss should be modified
      if(Ask<(OrderOpenPrice()-ssl) &&
         OrderStopLoss()>OrderOpenPrice()-((Ask+(OrderOpenPrice()-ssl))/2))   {
         s.tsl=NormalizeDouble(OrderOpenPrice()-((Ask+(OrderOpenPrice()-ssl))/2),Digits);
