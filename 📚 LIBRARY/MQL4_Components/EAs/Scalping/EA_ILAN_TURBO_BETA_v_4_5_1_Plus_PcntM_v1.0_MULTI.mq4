...
extern string _P_Profit = " ----- Profit ----- ";
extern bool   ByPercentProfit = true;            //Закрыть ордера по проценту прибыли
extern double PercentProfit   = 1.00;            // Процент профита
extern bool   ByMoneyProfit   = false;           // Закрыть ордера по достижении прибыли
extern double MoneyProfit     = 20.00;           // Прибыль в валюте депозита
//При слежении за несколькими парами - желательно включать только на одном советнике (одна пара)
extern bool   AllPair         = true;            // Все пары
extern bool   ShowComment     = false;            // Показывать комментарий
...
static double OldBalance = -1;
   if (ShowComment) {
      string st="PercentProfit="+DoubleToStr(PercentProfit, 2)
               ...
               +"  Прибыль="+DoubleToStr((AccountEquity()/AccountBalance()-1)*100, 2)+" %"
               ;
      Comment(st);
    } else Comment("");

   //Есть ордера?
   if(ByPercentProfit || ByMoneyProfit) {
      bool NoOrders = true;
      for (int i = OrdersTotal() - 1; i >= 0; i--) {
         if (!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
         // Если не все пары - пропускаем ордера не относящиеся к нашей валюте
         if(!AllPair && OrderSymbol() != Symbol()) continue;
         //Любой символ и наши магики
         if (OrderMagicNumber() == MagicNumber_1 ||
                OrderMagicNumber() == MagicNumber) NoOrders = false;
      }
      if(NoOrders) OldBalance = AccountBalance(); //Нет ордеров - запомним баланс.
      
      if (ByPercentProfit && OldBalance != -1 && AccountEquity()>=OldBalance *(1+PercentProfit/100)) {
         Print("!!! Кроем ВСЕ!!! OldBalance = ", OldBalance," Equity = ",AccountEquity()," ",(AccountEquity()-OldBalance)/OldBalance*100,"%");
         ClosePosFirstProfit();
         return; 
      }
      if (ByMoneyProfit && OldBalance != -1 && AccountEquity()>=OldBalance + MoneyProfit) {
         Print("!!! Кроем ВСЕ!!! OldBalance = ", OldBalance," Equity = ",AccountEquity()," Прибыль ", AccountEquity()-OldBalance,AccountCurrency());
         ClosePosFirstProfit();
         return; 
      }
   }
...
void ClosePosFirstProfit() {
  ...
  for (i=k-1; i>=0; i--) {
    if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
      ...
      if ((OrderMagicNumber() == MagicNumber_1 || 
            OrderMagicNumber() == MagicNumber) && 
               (OrderType()==OP_BUY || OrderType()==OP_SELL)) ClosePosBySelect();
    }
  }
}
//+----------------------------------------------------------------------------+
