...
extern double FridayNightHour=22;
            ...
            if(Bid-OrderOpenPrice()>Point*TrailingStop) {
               ts = Bid-Point*TrailingStop;
				...
				if((OrderStopLoss()<ts) || (OrderStopLoss()==0)) {
			      result=OrderModify(OrderTicket(),OrderOpenPrice(),ts,OrderTakeProfit(),0,Blue);
					err = GetLastError();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   if(InitialStopLoss>0) { ts = Bid+(InitialStopLoss*Point);
      ...
      //ticket=OrderSendExtended(Symbol(),OP_BUY,Lots,Ask,Slippage,ts,tp,setup,MagicNumber,0,Blue);
      ticket=OrderSendExtended(Symbol(),OP_SELL,Lots,Bid,Slippage,ts,tp,setup,MagicNumber,0,Red);
