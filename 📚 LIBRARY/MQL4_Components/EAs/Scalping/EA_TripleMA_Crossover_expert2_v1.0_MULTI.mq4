...
//+---------------------------------------------------+
extern int FastMA_Method = 0; //0=sma, 1=ema, 2=smma, 3=lwma , 4=LSMA
...
extern int MiddleMA_Method = 0; //0=sma, 1=ema, 2=smma, 3=lwma, 4=LSMA
...
extern int SlowMA_Method = 0; //0=sma, 1=ema, 2=smma, 3=lwma, 4=LSMA
...
bool CheckExitCondition(string TradeType)
{
   ...
   if (FastMA_Method == 4)
   {
     fMA = LSMA(FastMAPeriod,FastMA_AppliedPrice,0,SignalCandle);
   }
   else
   {
     fMA = iMA(NULL, 0, FastMAPeriod, FastMA_Shift, FastMA_Method, FastMA_AppliedPrice, SignalCandle);
   }
   if (SlowMA_Method == 4)
   {
     mMA = LSMA(MiddleMAPeriod,MiddleMA_AppliedPrice,0,SignalCandle);
   }
   else
   {
     mMA = iMA(NULL, 0, MiddleMAPeriod, MiddleMA_Shift, MiddleMA_Method, FastMA_AppliedPrice, SignalCandle);
   }
...
bool CheckEntryCondition(string TradeType)
{
   ...
   if (SlowMA_Method == 4)
   {
     sMA = LSMA(SlowMAPeriod,SlowMA_AppliedPrice,0,SignalCandle);
   }
   else
   {
     sMA = iMA(NULL, 0, SlowMAPeriod, SlowMA_Shift, SlowMA_Method, FastMA_AppliedPrice, SignalCandle);
   }
...
bool CheckValidUserInputs()
{
   ...
   {
     Alert("FastMA_Method requires a value from 0 to 4",'\n','\r',"You entered ",FastMA_Method);
     return(true);
   }
   if (CheckMAMethod(MiddleMA_Method))
   {
     Alert("MiddleMA_Method requires a value from 0 to 4",'\n','\r',"You entered ",MiddleMA_Method);
     return(true);
   }
   if (CheckMAMethod(SlowMA_Method))
   {
     Alert("SlowMA_Method requires a value from 0 to 4",'\n','\r',"You entered ",SlowMA_Method);
     return(true);
   }
