...
//---- 
   ...
   if (OpenOrders<1) 
   {
	 ...
	 // if (iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,0)<iMACD(NULL,0,14,26,9,PRICE_CLOSE,MODE_MAIN,1)) { myOrderType=1; }//SELL
	   if (iSAR(NULL,0,0.001,0.1,1) <= Low[1]  && iSAR(NULL,0,0.001,0.1,0) > High[0]) { myOrderType=1; }//SELL
	   if (iSAR(NULL,0,0.001,0.1,1) >= High[1] && iSAR(NULL,0,0.001,0.1,0) < Low[0] ) { myOrderType=2; }//BUY
