...
string key = "Flexy The Dragon v2.0";
...
input bool use_autolot          = true;            // Use Auto Lot Size? (calculated by auto lot Balance)
...
extern double gs_multiplicator = 1.3;              // Grid Multiplier (only for martingale)
sinput string gap21; // .
...
sinput string gap30; // .
extern double profit_lock        = 1.0;            // Profit Lock / Trail Stop
sinput string profit_lock2;                        // (safe profit per grid step, 0.01 = 1%)
extern double cycleEquityRisk    = 0.0;            // Cycle Risk / Stop Loss
sinput string cycleEquityRisk2;                    // (close orders of grid cycle, 0.01 = 1%)
sinput string gap31; // .
extern double equity_warning     = 0.0;            // Yoga Mode / Equity Warning 
sinput string equity_warning2;                     // (stop opening orders [yellow], 0.01 = 1%)
extern double account_risk       = 1.0;            // Shut Down / Account Risk
sinput string account_risk2;                       // (close all open orders [red], 0.01 = 1%)
sinput string gap32; // .
extern int grid_partially_close  = 5;              // Partial Closing
sinput string grid_partially_close2;               // (number of orders to close last + 1st trade of cycle)
extern bool hedging_active       = false;          // Cycle Hedging
sinput string hedging_active2;                     // (hedge entire cycle, if max. positions reached)
extern int profit_chasing        = 0;              // Profit Chasing
sinput string profit_chasing2;                     // (number of profitable orders to chase max. profit)
sinput string gap33; // .
...
extern bool C1_Exit          = true;                    // Exit other open orders? (experimental)
sinput string gap61; // .
...
extern bool C2_Exit          = true;                    // Exit other open orders? (experimental)
sinput string gap62; // .
...
extern bool C3_Exit          = true;                    // Exit other open orders? (experimental)
...
int btnWidth = 100;            // width of smallest buttons
int btnHeight = 20;           // height of all buttons
int btnGap = 3;               // gap, between buttons
int btnLeftAxis = 250;        // distance of button from left screen border
int btnTopAxis = 4;           // distance of button from top screen border
...
// implement buttons
   ...
   DrawButton("btnstopNextCycle", "Stop Next Cycle", btnLeftAxis + 2 * btnNextLeft, btnTopAxis, (int)(btnWidth * 1.3), btnHeight, false, colNeutral, clrBlack);
   DrawButton("btnrestAndRealize", "Rest & Realize", btnLeftAxis + 2 * btnNextLeft, btnTopAxis + btnNextTop, (int)(btnWidth * 1.3), btnHeight, false, colNeutral, clrBlack);
   DrawButton("btnStopAll", "Stop & Close", btnLeftAxis + 2 * btnNextLeft, btnTopAxis + 2 * btnNextTop, (int)(btnWidth * 1.3), btnHeight, false, colNeutral, clrBlack);
   // DrawButton("btnShowComment", "Show/Hide Comment", 600, btnTopAxis, btnWidth * 1.3, btnHeight, false, colNeutral, colCodeYellow); // button not used
   ...
   switch(accountState)
     {
      ...
      default:
         ...
         SetButtonColor("btnCloseAllBuys", colCodeGreen, colFontLight);
         SetButtonColor("btnCloseAllSells", colCodeGreen, colFontLight);
...
// #047: trade info panel (right upper corner)
      ...
      Write("panel_1_10", "DD: -" + DoubleToString((max_equity - AccountEquity()), 2) + " " + ter_currencySymbol + " (-" + DoubleToString((max_equity - AccountEquity()) / max_equity * 100, 2) + " %)", 5, 190, "Roboto Condensed", 14, clrGold);
      Write("panel_1_11", "Max: " + DoubleToString(max_float, 2) + " " + ter_currencySymbol + " (" + DoubleToString(max_float / max_equity * 100, 2) + " %)", 5, 213, "Roboto Condensed", 11, clrDarkGoldenrod);
...
if(show_settings) 
      ...
      string take_profit_str = DoubleToString(take_profit, 1);
      Write("panel_1_44", "Take Profit: " + take_profit_str, 5, 740, "Roboto Condensed", 10, colFontGray);
      ...
      if(grid_partially_close != 0) 
      Write("panel_1_50", "Partial Closing: " + (string)grid_partially_close, 5, 820, "Roboto Condensed", 10, colFontGray); else 
...
if(show_indicators) 
      ...
      double currentBuyBufferValue1 = iCustom(NULL, 0, C1_Name, C1_BuyBuffer, 0);
      double currentSellBufferValue1 = iCustom(NULL, 0, C1_Name, C1_SellBuffer, 0);
      string buyText1 = "C1Buy("; string sellText1 = ", C1Sell(";
      if (currentBuyBufferValue1 == EMPTY_VALUE) {buyText1 += "--------";} else {buyText1 += DoubleToString(currentBuyBufferValue1, 3);}
      if (currentSellBufferValue1 == EMPTY_VALUE) {sellText1 += "--------";} else {sellText1 += DoubleToString(currentSellBufferValue1, 3);}
      string displayText1 = buyText1 + "), " + sellText1 + ")";
      Write("panel_1_47C1", displayText1, 900, 3, "Roboto Condensed", 9, colFontGray);
      
      double currentBuyBufferValue2 = iCustom(NULL, 0, C2_Name, C2_BuyBuffer, 0);
      double currentSellBufferValue2 = iCustom(NULL, 0, C2_Name, C2_SellBuffer, 0);
      string buyText2 = "C2Buy("; string sellText2 = ", C2Sell(";
      if (currentBuyBufferValue2 == EMPTY_VALUE) {buyText2 += "--------";} else {buyText2 += DoubleToString(currentBuyBufferValue2, 3);}
      if (currentSellBufferValue2 == EMPTY_VALUE) {sellText2 += "--------";} else {sellText2 += DoubleToString(currentSellBufferValue2, 3);}
      string displayText2 = buyText2 + "), " + sellText2 + ")";
      Write("panel_1_47C2", displayText2, 900, 15, "Roboto Condensed", 9, colFontGray);
      
      double currentBuyBufferValue3 = iCustom(NULL, 0, C3_Name, C3_BuyBuffer, 0);
      double currentSellBufferValue3 = iCustom(NULL, 0, C3_Name, C3_SellBuffer, 0);
      string buyText3 = "C3Buy("; string sellText3 = ", C3Sell(";
      if (currentBuyBufferValue3 == EMPTY_VALUE) {buyText3 += "--------";} else {buyText3 += DoubleToString(currentBuyBufferValue3, 3);}
      if (currentSellBufferValue3 == EMPTY_VALUE) {sellText3 += "--------";} else {sellText3 += DoubleToString(currentSellBufferValue3, 3);}
      string displayText3 = buyText3 + "), " + sellText3 + ")";
      Write("panel_1_47C3", displayText3, 900, 27, "Roboto Condensed", 9, colFontGray);
      ...
      Write("panel_1_47a", DoubleToString(deviation, 2) + " ~ BB("+ (string)BB_Period + "," + DoubleToString(BB_Dev,1) +")", 765, 3, "Roboto Condensed", 9, colFontGray);
      if (Use_BB == true) 
      Write("panel_1_47a", DoubleToString(deviation, 2) + " ~ BB("+ (string)BB_Period + "," + DoubleToString(BB_Dev,1) +")", 765, 3, "Roboto Condensed", 9, clrWhite);
      if (Use_BB == true && Conjunct_Idx == false) 
      Write("panel_1_47a", DoubleToString(deviation, 2) + " ~ BB("+ (string)BB_Period + "," + DoubleToString(BB_Dev,1) +")", 765, 3, "Roboto Condensed", 9, clrYellow);
      if (Use_BB == true && price > BBU) 
      Write("panel_1_47a", DoubleToString(deviation, 2) + " > BB("+ (string)BB_Period + "," + DoubleToString(BB_Dev,1) +")", 765, 3, "Roboto Condensed", 9, clrDodgerBlue);
      else if (Use_BB == true && price < BBL) 
      Write("panel_1_47a", DoubleToString(deviation, 2) + " < BB("+ (string)BB_Period + "," + DoubleToString(BB_Dev,1) +")", 765, 3, "Roboto Condensed", 9, clrCrimson);
      
      double stochK = iStochastic(NULL, 0, Stoch_K, Stoch_D, Stoch_Slowing, Stoch_Method, Stoch_Price, MODE_SIGNAL, Stoch_Shift);
      Write("panel_1_47b", DoubleToString(stochK, 1) + " ~ STO("+ (string)Stoch_K + "," + (string)Stoch_Upper + "/" + (string)Stoch_Lower +")", 630, 3, "Roboto Condensed", 9, colFontGray);
      if (Use_Stoch == true) 
      Write("panel_1_47b", DoubleToString(stochK, 1) + " ~ STO("+ (string)Stoch_K + ")" + (string)Stoch_Upper + "/" + (string)Stoch_Lower +")", 630, 3, "Roboto Condensed", 9, clrWhite);
      if (Use_Stoch == true && Conjunct_Idx == false) 
      Write("panel_1_47b", DoubleToString(stochK, 1) + " ~ STO("+ (string)Stoch_K + ")" + (string)Stoch_Upper + "/" + (string)Stoch_Lower +")", 630, 3, "Roboto Condensed", 9, clrYellow);
      if (Use_Stoch == true && stochK >= Stoch_Upper) 
      Write("panel_1_47b", DoubleToString(stochK, 1) + " > STO("+ (string)Stoch_K + ")" + (string)Stoch_Upper + "/" + (string)Stoch_Lower +")", 630, 3, "Roboto Condensed", 9, clrDodgerBlue);
      else if (Use_Stoch == true && stochK <= Stoch_Lower) 
      Write("panel_1_47b", DoubleToString(stochK, 1) + " < STO("+ (string)Stoch_K + ")" + (string)Stoch_Upper + "/" + (string)Stoch_Lower +")", 630, 3, "Roboto Condensed", 9, clrCrimson);
            
      double rsiValue = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, RSI_Shift);
      Write("panel_1_47c", DoubleToString(rsiValue, 1) + " ~ RSI("+ (string)RSI_Period + ")" + (string)RSI_Upper + "/" + (string)RSI_Lower +")", 500, 3, "Roboto Condensed", 9, colFontGray);
      if (Use_RSI) 
      Write("panel_1_47c", DoubleToString(rsiValue, 1) + " ~ RSI("+ (string)RSI_Period + ")" + (string)RSI_Upper + "/" + (string)RSI_Lower +")", 500, 3, "Roboto Condensed", 9, clrWhite);
      if (Use_RSI && Conjunct_Idx == false) 
      Write("panel_1_47c", DoubleToString(rsiValue, 1) + " ~ RSI("+ (string)RSI_Period + ")" + (string)RSI_Upper + "/" + (string)RSI_Lower +")", 500, 3, "Roboto Condensed", 9, clrYellow);
      if (Use_RSI == true && rsiValue >= RSI_Upper) 
      Write("panel_1_47c", DoubleToString(rsiValue, 1) + " > RSI("+ (string)RSI_Period + ")" + (string)RSI_Upper + "/" + (string)RSI_Lower +")", 500, 3, "Roboto Condensed", 9, clrDodgerBlue);
      else if (Use_RSI == true && rsiValue <= RSI_Lower) 
      Write("panel_1_47c", DoubleToString(rsiValue, 1) + " < RSI("+ (string)RSI_Period + ")" + (string)RSI_Upper + "/" + (string)RSI_Lower +")", 500, 3, "Roboto Condensed", 9, clrCrimson);
      
      double cciValue = iCCI(NULL, 0, CCI_Period, PRICE_CLOSE, CCI_Shift);
      Write("panel_1_47d", DoubleToString(cciValue, 1) + " ~ CCI("+ (string)CCI_Period + ")" + (string)CCI_Upper + "/" + (string)CCI_Lower +")", 355, 3, "Roboto Condensed", 9, colFontGray);
      if (Use_CCI == true) 
      Write("panel_1_47d", DoubleToString(cciValue, 1) + " ~ CCI("+ (string)CCI_Period + ")" + (string)CCI_Upper + "/" + (string)CCI_Lower +")", 355, 3, "Roboto Condensed", 9, clrWhite);
      if (Use_CCI == true && Conjunct_Idx == false) 
      Write("panel_1_47d", DoubleToString(cciValue, 1) + " ~ CCI("+ (string)CCI_Period + ")" + (string)CCI_Upper + "/" + (string)CCI_Lower +")", 355, 3, "Roboto Condensed", 9, clrYellow);
      if (Use_CCI == true && cciValue >= CCI_Upper) 
      Write("panel_1_47d", DoubleToString(cciValue, 1) + " > CCI("+ (string)CCI_Period + ")" + (string)CCI_Upper + "/" + (string)CCI_Lower +")", 355, 3, "Roboto Condensed", 9, clrDodgerBlue);
      else if (Use_CCI == true && cciValue <= CCI_Lower) 
      Write("panel_1_47d", DoubleToString(cciValue, 1) + " < CCI("+ (string)CCI_Period + ")" + (string)CCI_Upper + "/" + (string)CCI_Lower +")", 355, 3, "Roboto Condensed", 9, clrCrimson);
