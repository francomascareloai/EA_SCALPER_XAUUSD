//+------------------------------------------------------------------+
//|                        SMC Manual EA limit and/or stop order     |
//+------------------------------------------------------------------+
...
extern double  DecreaseFactor    =0;//0 for no decrease in lotsize
extern double  Slippage          =3;
extern int     UseTrailingStop   =1;//>0 enables trailing stop
extern double  TrailingStop      =20;
extern int     OneTradePerDay    =1;//>0 LIMITS(restricts) trades to ONE PER DAY
extern int     ExtendLines       =1;//>0 for extended trendline
extern int     Stoporders        =1;//>0 to draw stoporder trendlines
extern int     Limitorders       =1;//>0 to draw limitorder trendlines
...
if (Stoporders>0)//Stop order enabled
{
...
ObjectSet("BuyStop",10,ExtendLines);
...
ObjectSet("SellStop",10,ExtendLines);
...
ObjectSet("BuyStopSL",10,ExtendLines);
...
ObjectSet("SellStopSL",10,ExtendLines);
...
ObjectSet("BuyStopTP",10,ExtendLines);
...
ObjectSet("SellStopTP",10,ExtendLines);
ObjectSetText("SellStopTP","SellStopTP");
}//if Stop order enabled

//###########################  LIMIT ORDER TRENDLINES ######################

if (Limitorders>0)//limit orders enabled
{
...
ObjectSet("BuyLimit",10,ExtendLines);
...
ObjectSet("SellLimit",10,ExtendLines);
...
ObjectSet("BuyLimitSL",10,ExtendLines);
...
ObjectSet("SellLimitSL",10,ExtendLines);
...
ObjectSet("BuyLimitTP",10,ExtendLines);
...
ObjectSet("SellLimitTP",10,ExtendLines);
ObjectSetText("SellLimitTP","SellLimitTP");
}//if limit orders enabled
