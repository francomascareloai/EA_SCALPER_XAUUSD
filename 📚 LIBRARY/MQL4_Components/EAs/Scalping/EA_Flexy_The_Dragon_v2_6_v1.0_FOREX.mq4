...
extern int partial_breaking      = 0;              // 2# Partial Breaking
sinput string partial_breaking_info;               // (number of orders to start closing 1st grid trades in loss)
extern double reduce_tp          = 0.0;            // 3# Profit Reducing
...
void closeOrdersInLoss() {
    ...
    if (partial_breaking >   0) {
        ...
        if (totalOrders > partial_breaking) {
            ...
            for (int i =   0; i < buys; i++) {
                ...
                // Break the loop if the total number of orders is equal to the partial_breaking threshold
                if (totalOrders <= partial_breaking) break;
            }
