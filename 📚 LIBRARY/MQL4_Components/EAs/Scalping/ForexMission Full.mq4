...
int init() {
   ...
   ObjectSetText("Lable1", G_text_1472, 12, "Arial", DarkOrange);
   ...
   ObjectSetText("Lable", G_text_1464, 10, "Arial", DarkOrange);
   if (Gi_1480) {
      if (KEY != "") {
         Gi_1492 = 0;
...
/*   if (!IsDemo())
   ...
   for (G_pos_392 = OrdersTotal() - 1; G_pos_392 >= 0; G_pos_392--) {
      esh_ri = OrderSelect(G_pos_392, SELECT_BY_POS, MODE_TRADES);
   ...
   if (Gi_424) {
      if (Gi_360 == TRUE) {
         for (G_pos_392 = OrdersTotal() - 1; G_pos_392 >= 0; G_pos_392--) {
            esh_ri = OrderSelect(G_pos_392, SELECT_BY_POS, MODE_TRADES);
            if (OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber_Hilo) continue;
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo) msoi_i = OrderModify(OrderTicket(), G_price_312, OrderStopLoss(), G_price_280, 0, Yellow);
            Gi_424 = FALSE;
         }
      }
   }
   ...
   if (Gi_656 != Time[0]) {
      ...
      for (G_pos_676 = OrdersTotal() - 1; G_pos_676 >= 0; G_pos_676--) {
         esh_ri = OrderSelect(G_pos_676, SELECT_BY_POS, MODE_TRADES);
   ...
   if (G_datetime_728 != iTime(NULL, G_timeframe_496, 0)) {
      ...
      for (int Li_1272 = Li_1264; Li_1272 >= 1; Li_1272--) {
         esh_ri = OrderSelect(Li_1272 - 1, SELECT_BY_POS, MODE_TRADES);
         if (OrderSymbol() != Symbol() || OrderMagicNumber() != G_magic_176_15) continue;
   ...
   if (Gi_708) {
      if (Gi_644 == TRUE) {
         for (G_pos_676 = OrdersTotal() - 1; G_pos_676 >= 0; G_pos_676--) {
            esh_ri = OrderSelect(G_pos_676, SELECT_BY_POS, MODE_TRADES);
            if (OrderSymbol() != Symbol() || OrderMagicNumber() != G_magic_176_15) continue;
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == G_magic_176_15) msoi_i = OrderModify(OrderTicket(), G_price_596, OrderStopLoss(), G_price_564, 0, Yellow);
            Gi_708 = FALSE;
         }
      }
   }
   ...
   if (Gi_940 != Time[0]) {
      ...
      for (G_pos_960 = OrdersTotal() - 1; G_pos_960 >= 0; G_pos_960--) {
         esh_ri = OrderSelect(G_pos_960, SELECT_BY_POS, MODE_TRADES);
   ...
   if (G_datetime_1012 != iTime(NULL, G_timeframe_784, 0)) {
      ...
      for (int Li_1340 = Li_1332; Li_1340 >= 1; Li_1340--) {
         esh_ri = OrderSelect(Li_1340 - 1, SELECT_BY_POS, MODE_TRADES);
         if (OrderSymbol() != Symbol() || OrderMagicNumber() != G_magic_176_16) continue;
   ...
   if (Gi_992) {
      if (Gi_928 == TRUE) {
         for (G_pos_960 = OrdersTotal() - 1; G_pos_960 >= 0; G_pos_960--) {
            esh_ri = OrderSelect(G_pos_960, SELECT_BY_POS, MODE_TRADES);
            if (OrderSymbol() != Symbol() || OrderMagicNumber() != G_magic_176_16) continue;
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == G_magic_176_16) msoi_i = OrderModify(OrderTicket(), G_price_880, OrderStopLoss(), G_price_848, 0, Yellow);
            Gi_992 = FALSE;
         }
      }
   }
...
int f0_4() {
   ...
   for (int pos_4 = OrdersTotal() - 1; pos_4 >= 0; pos_4--) {
      esh_ri = OrderSelect(pos_4, SELECT_BY_POS, MODE_TRADES);
      if (OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber_Hilo) continue;
      if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo)
...
void f0_24() {
   for (int pos_0 = OrdersTotal() - 1; pos_0 >= 0; pos_0--) {
      esh_ri = OrderSelect(pos_0, SELECT_BY_POS, MODE_TRADES);
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo) {
            if (OrderType() == OP_BUY) std_ts = OrderClose(OrderTicket(), OrderLots(), Bid, slip_Hilo, Blue);
            if (OrderType() == OP_SELL) std_ts = OrderClose(OrderTicket(), OrderLots(), Ask, slip_Hilo, Red);
         }
         Sleep(1000);
      }
   }
         ...
         if (OrderSelect(pos_36, SELECT_BY_POS, MODE_TRADES)) {
            ...
            if (OrderSymbol() == Symbol() || OrderMagicNumber() == MagicNumber_Hilo) {
               if (OrderType() == OP_BUY) {
                  ...
                  if (order_stoploss_20 == 0.0 || (order_stoploss_20 != 0.0 && price_28 > order_stoploss_20)) msoi_i = OrderModify(OrderTicket(), A_price_8, price_28, OrderTakeProfit(), 0, Aqua);
               }
               if (OrderType() == OP_SELL) {
                  ...
                  if (order_stoploss_20 == 0.0 || (order_stoploss_20 != 0.0 && price_28 < order_stoploss_20)) msoi_i = OrderModify(OrderTicket(), A_price_8, price_28, OrderTakeProfit(), 0, Red);
               }
            }
            Sleep(1000);
         }
...
double f0_32() {
   ...
   for (int pos_24 = OrdersTotal() - 1; pos_24 >= 0; pos_24--) {
      esh_ri = OrderSelect(pos_24, SELECT_BY_POS, MODE_TRADES);
      if (OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber_Hilo) continue;
...
void f0_18() {
   for (int pos_0 = OrdersTotal() - 1; pos_0 >= 0; pos_0--) {
      ...
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == G_magic_176_15) {
            if (OrderType() == OP_BUY) std_ts = OrderClose(OrderTicket(), OrderLots(), Bid, slip_15, Blue);
            if (OrderType() == OP_SELL) std_ts = OrderClose(OrderTicket(), OrderLots(), Ask, slip_15, Red);
         }
         Sleep(1000);
      }
   }
...
void f0_0() {
   for (int pos_0 = OrdersTotal() - 1; pos_0 >= 0; pos_0--) {
      ...
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == G_magic_176_16) {
            if (OrderType() == OP_BUY) std_ts = OrderClose(OrderTicket(), OrderLots(), Bid, slip_16, Blue);
            if (OrderType() == OP_SELL) std_ts = OrderClose(OrderTicket(), OrderLots(), Ask, slip_16, Red);
         }
         Sleep(1000);
      }
   }
