...
20 Nov 2014 by Capella
Version edu 1.8
- Added Monday Start Hour and Friday Close Hour (Note: Broker server time)
- Added two sessions of trading hours (Note: Broker server time)
*/


#property copyright "Copyright (c) 2013 waygrow"
...
extern string L_ = "=== WAYGROW Settings ===";
extern int Filter = 20.0;
extern int MagicNumber = 111;
extern string TradeComment = "WG 1.8 edu";
extern int StopLoss = 23;
extern int NoTrade_Profit_Yet_Filter = 20; 
extern int NoTrade_Profit_Yet_StopLoss = 23;
extern int NoTrade_Profit_Yet_Distance = 20;
extern int InTrade_Profit_Filter = 20; 
extern int InTrade_Profit_StopLoss = 23;
extern int InTrade_Profit_Distance = 20;
extern int TrailingStop = 30;
extern int TrailingStep = 12;
extern int BreakEven    = 3;
extern int movestopto   = 10;
extern bool UseHiddenTP = FALSE;
extern int TakeProfit_Hidden = 0.0;
...
extern int Limit = 20.0; 
extern int Distance = 0.0;  
extern int MAPeriod = 3;
extern int MAMethod = 2;
extern string L__ = "=== Trading Hour Settings ===";  // Note broker server time
extern string L___ = "-- Session 1: Start & End Hours --";
extern int Session1_StartHour = 0;					// First session start hour. Time is broker server time
extern int Session1_EndHour = 12;				   // First session end hour. Time is broker server time
extern string L____ = "--- Session 2: Start & End Hours ---";
extern int Session2_StartHour = 12;			   // Second session start hour. Time is broker server time
extern int Session2_EndHour = 24;				   // Second session end hour. Time is broker server time
extern string L_____ = "--- Monday Start Hour ---";
extern int Monday_StartHour = 0;						// Trading will not start on Monday before this time. Broker server time.
extern string L______ = "--- Friday Close Hour ---";
extern int Friday_EndHour = 24;					   // Trading will end on Friday at this time. Broker server time.
/*
...
extern int Env_period = 5;
extern double Env_deviation = 0.05;
...
int f0_4() 
{
	return ( TradeTime () );  // Added by Capella
...
int OrderCount(){                       // we counting our order here
   ...
   int n;
      n= OrderSelect(i,SELECT_BY_POS,MODE_TRADES);  // is it right our order?
...
void MoveTrailingStop() {
   ...
   for(int cnt=0;cnt<OrdersTotal();cnt++)  {
      m = OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
         ...
         if(OrderType()==OP_BUY)  {
            if(TrailingStop>0&&NormalizeDouble(Ask-TrailingStep*point,Digits)>NormalizeDouble(OrderOpenPrice()+TrailingStop*point,Digits))  {                 
               if((NormalizeDouble(OrderStopLoss(),Digits)<NormalizeDouble(Bid-TrailingStop*point,Digits))||(OrderStopLoss()==0))   {
                  m=OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(Bid-TrailingStop*point,Digits),OrderTakeProfit(),0,Blue);
            ...
            if(TrailingStop>0&&NormalizeDouble(Bid+TrailingStep*point,Digits)<NormalizeDouble(OrderOpenPrice()-TrailingStop*point,Digits))  {                 
               if((NormalizeDouble(OrderStopLoss(),Digits)>(NormalizeDouble(Ask+TrailingStop*point,Digits)))||(OrderStopLoss()==0))  {
                  m=OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(Ask+TrailingStop*point,Digits),OrderTakeProfit(),0,Red);
            ...
            if(BreakEven>0)   {
               if(NormalizeDouble((Bid-OrderOpenPrice()),Digits)>BreakEven*point)    {
                  if(NormalizeDouble((OrderStopLoss()-OrderOpenPrice()),Digits)<0) {
                     s=OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(OrderOpenPrice()+movestopto*point,Digits),OrderTakeProfit(),0,Blue);
            ...
            if(BreakEven>0) {
               if(NormalizeDouble((OrderOpenPrice()-Ask),Digits)>BreakEven*point)    {
                  if(NormalizeDouble((OrderOpenPrice()-OrderStopLoss()),Digits)<0) {
                     s=OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(OrderOpenPrice()-movestopto*point,Digits),OrderTakeProfit(),0,Red);
