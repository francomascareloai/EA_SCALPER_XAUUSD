...
#include "..\..\..\..\Include\MCP_Integration_Library.mqh"
...
void OnTick()
{
    ...
    // FTMO Compliance Check - CRITICAL FIRST CHECK
    ...
    // Check for new trading opportunities (only if FTMO compliant)
    if(!PositionsTotal() || PositionsTotal() < 2) // Max 2 positions for FTMO
    {
...
// ==================================================================
// MCP Integration - Implementations (self-contained)
// Placed here to ensure types/structs are available when compiling.
// ==================================================================

bool InitializeMCPIntegration()
{
    // Mark AI optimization as available and initialize baselines
    g_last_ai_optimization = TimeCurrent();
    g_ai_optimized_confluence = InpConfluenceThreshold;
    g_ai_optimized_risk = InpRiskPercent;
    g_ai_optimization_active = true;
    g_ai_optimization_count = 0;
    g_ai_recommendations = "";
    return true;
}
...
// Simple periodic auto-tuning using account performance and daily state
void CheckAIOptimization()
{
    if(!g_ai_optimization_active) return;

    // Run at most every InpAIOptimizationInterval minutes
    if((TimeCurrent() - g_last_ai_optimization) < (InpAIOptimizationInterval * 60))
        return;

    g_last_ai_optimization = TimeCurrent();
    g_ai_optimization_count++;

    SPerformanceMetrics pm = CalculatePerformanceMetrics();

    // Adjust confluence threshold conservatively: tighten if equity pressure, relax if strong performance
    double adj = 0.0;
    if(g_ftmo_compliance.daily_limit_breached || g_ftmo_compliance.drawdown_limit_breached)
        adj = +5.0; // tighten (require more confluence)
    else if(pm.win_rate > 0.75 && pm.total_profit > 0)
        adj = -3.0; // slightly relax on good streak

    g_ai_optimized_confluence = MathMax(60.0, MathMin(95.0, InpConfluenceThreshold + adj));

    // Risk fine-tuning within ultra conservative bounds
    double risk_adj = 0.0;
    if(pm.win_rate < 0.55) risk_adj = -0.2;
    else if(pm.win_rate > 0.8) risk_adj = +0.1;
    g_ai_optimized_risk = MathMax(0.2, MathMin(1.0, InpRiskPercent + risk_adj));
}

double GetEffectiveConfluenceThreshold()
{
    return (g_ai_optimization_active ? g_ai_optimized_confluence : InpConfluenceThreshold);
}

// Out-param version declared in header
bool GenerateAIEnhancedConfluenceSignal(SConfluenceSignal &out_signal)
{
    out_signal = GenerateConfluenceSignal();
    if(!g_ai_optimization_active) return true;

    // Confidence nudges based on institutional alignment and current FTMO state
    double alignment = CalculateInstitutionalAlignment();
    double bonus = 0.0;
    if(alignment > 0.85) bonus += 3.0;
    if(!g_ftmo_compliance.daily_limit_breached && !g_ftmo_compliance.drawdown_limit_breached)
        bonus += 1.0;

    out_signal.confidence_score = MathMin(100.0, out_signal.confidence_score + bonus);
    return true;
}

// Convenience wrapper to preserve original call style (by value)
SConfluenceSignal GenerateAIEnhancedConfluenceSignal()
{
    SConfluenceSignal s;
    GenerateAIEnhancedConfluenceSignal(s);
    return s;
}

// Emergency checks beyond FTMO core (spread spikes / weekend / equity shocks)
bool CheckEnhancedEmergencyConditions()
{
    if(g_ftmo_compliance.trading_halted) return true;

    // Spread guard
    double spread_pts = (double)symbol_info.Spread();
    if(spread_pts > InpMaxSpread * 1.5) // 50% above configured max
        return true;

    // Equity shock guard (intra-bar)
    static double last_equity = 0.0;
    double eq = account_info.Equity();
    if(last_equity > 0 && eq < last_equity * 0.98) // >2% sudden drop
        return true;
    last_equity = eq;

    // Weekend/news protections already covered elsewhere
    return false;
}

// Basic AI gate using existing metrics (kept conservative)
bool ValidateTradeWithAI(const SConfluenceSignal& signal, bool &approved)
{
    double thr = GetEffectiveConfluenceThreshold();
    approved = (signal.confidence_score >= thr && signal.risk_reward_ratio >= InpBreakevenRR);
    return true;
}
...
void CalculateTradeParameters(SConfluenceSignal& signal, ENUM_SIGNAL_TYPE signal_type)
{
    ...
    double sl_distance = InpStopLoss * _Point;
    double tp_distance = InpTakeProfit * _Point;
...
bool ValidateNewsFilter()
{
    ...
    // Simplified news filter - avoid major news times
    if(dt.hour == 12 && dt.min >= 30) return false; // US CPI
    if(dt.hour == 13 && dt.min <= 30) return false; // CPI aftermath
    if(dt.hour == 18 && dt.min >= 0 && dt.min <= 30) return false; // FOMC
...
double CalculateLotSize(const SConfluenceSignal& signal)
{
    ...
    if(InpLotMethod == LOT_PERCENT_RISK)
    {
        ...
        {
            lot_size = risk_amount / ((sl_distance / _Point) * tick_value);
        }
    }
...
void ExecutePartialProfitStrategy(SAutonomousPositionData* pos_data)
{
    ...
    if(pos_data.current_r_multiple >= required_r)
    {
        ...
        if(pos_data.position_type == POSITION_TYPE_BUY)
        {
            success = trade.Sell(partial_volume, _Symbol, 0, 0, 0, "Partial TP");
        }
        else
        {
            success = trade.Buy(partial_volume, _Symbol, 0, 0, 0, "Partial TP");
        }
