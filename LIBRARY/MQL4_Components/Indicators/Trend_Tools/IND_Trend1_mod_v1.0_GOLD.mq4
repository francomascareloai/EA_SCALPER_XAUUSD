...
//+------------------------------------------------------------------+
//Modified, 26/Aug/2021, by jeanlouie, www.forexfactory.com/jeanlouie
// - added crossover arrows on chart
//Modified, 26/Aug/2021, by jeanlouie, www.forexfactory.com/jeanlouie
// - obj arrows to buffer arrows
...
//---- indicator line
   ...
   SetIndexBuffer(2,a_up);
   SetIndexBuffer(3,a_dn);
//----
...
{  
  ...
  //ObjectDelete(0,"thing!"+IntegerToString(i));
  
  if(value[i+1]>value2[i+1] && value[i+2]<value2[i+2]){
   a_up[i+1] = value[i+1];
   //ObjectCreate(0,"thing!"+IntegerToString(i),OBJ_ARROW,0,Time[i+1],Low[i+1]);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_ARROWCODE,233);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_ANCHOR,ANCHOR_TOP);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_COLOR,clrRoyalBlue);
  }
  if(value[i+1]<value2[i+1] && value[i+2]>value2[i+2]){
   a_dn[i+1] = value2[i+1];
   //ObjectCreate(0,"thing!"+IntegerToString(i),OBJ_ARROW,0,Time[i+1],High[i+1]);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_ARROWCODE,234);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_ANCHOR,ANCHOR_BOTTOM);
   //ObjectSetInteger(0,"thing!"+IntegerToString(i),OBJPROP_COLOR,clrOrangeRed);
  }
}  
