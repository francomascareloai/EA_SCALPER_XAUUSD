...
void check_breakouts_and_tp5(int i)
{
   ...
   double low = Low[i];
   string objname;
   static datetime lastBuyArrowDate = 0;
   static datetime lastSellArrowDate = 0;
   static datetime lastTP5CrossDate = 0;
   string currentDate = TimeToStr(currentTime, TIME_DATE);

   if (ShowBreakoutArrow && currentTime >= tSessionStart && currentTime <= tSessionEnd) {
      string lastBuyDate = TimeToStr(lastBuyArrowDate, TIME_DATE);
      string lastSellDate = TimeToStr(lastSellArrowDate, TIME_DATE);

      // Sell Arrow: Display red arrow if previous candle is above SellEntry and current low crosses below SellEntry
      if (low <= SellEntry && iLow(NULL, 0, i+1) > SellEntry && currentDate != lastBuyDate) {
         objname = objPrefix + "SellArrow-" + TimeToStr(currentTime, TIME_DATE | TIME_MINUTES);
         ObjectCreate(objname, OBJ_ARROW_DOWN, 0, currentTime, SellEntry);
         ...
         lastSellArrowDate = currentTime;
      }
      // Buy Arrow: Display green arrow if previous candle is below BuyEntry and current high crosses above BuyEntry
      if (high >= BuyEntry && iHigh(NULL, 0, i+1) < BuyEntry && currentDate != lastSellDate) {
         objname = objPrefix + "BuyArrow-" + TimeToStr(currentTime, TIME_DATE | TIME_MINUTES);
         ObjectCreate(objname, OBJ_ARROW_UP, 0, currentTime, BuyEntry);
         ObjectSetInteger(0, objname, OBJPROP_COLOR, BuyArrowColor);
         ObjectSetInteger(0, objname, OBJPROP_WIDTH, BreakoutArrowSize);
         ObjectSetInteger(0, objname, OBJPROP_ARROWCODE, 233);
         lastBuyArrowDate = currentTime;
      }
   }

   if (ShowTP5Cross && TP5Factor > 0 && currentTime >= tSessionStart && currentTime <= tSessionEnd) {
      string lastCrossDate = TimeToStr(lastTP5CrossDate, TIME_DATE);

      if (High[i] >= BuyTP5 && iHigh(NULL, 0, i+1) < BuyTP5 && currentDate != lastCrossDate) {
         ...
         lastTP5CrossDate = currentTime;
      }
      if (Low[i] <= SellTP5 && iLow(NULL, 0, i+1) > SellTP5 && currentDate != lastCrossDate) {
