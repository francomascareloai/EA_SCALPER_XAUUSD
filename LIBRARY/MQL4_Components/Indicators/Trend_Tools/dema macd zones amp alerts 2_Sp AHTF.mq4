...
extern string UniqueID  = "MACD zones H";
...
double macd[], signal[], colors[], times[];
...
//AHTF Timeframe Timeframe template copy and paste end13
   ...
   wind = WindowFind(indicatorFileName);
         ...
         string name = UniqueID+":"+Time[i];
            ...
            if (colors[i]!=colors[i+1] || i==0)
            {
               ...
               if (times[i+1]<Time[0])
               {
                  ...
                  ObjectCreate(name,OBJ_RECTANGLE,wind,times[i+1],0,lastTime,MaxValue);
