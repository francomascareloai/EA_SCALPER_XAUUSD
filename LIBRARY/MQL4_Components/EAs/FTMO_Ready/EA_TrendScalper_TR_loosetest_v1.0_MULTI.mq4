   ...
   if (Opentrades==0)  //and iATR(5,2)<StopLoss*Point 
     ...
     if (LastBarChecked == Time[0]) 
         return(0); 
  	      ...
  	      if (! High[1]>iBands(NULL,0,100,1,0,PRICE_CLOSE,MODE_UPPER,1) && ! Low[1]<iBands(NULL,0,100,1,0,PRICE_CLOSE,MODE_LOWER,1)) 
  	         return(0);
  	      }
     ...
     && High[1]>iBands(NULL,0,100,1,0,PRICE_CLOSE,MODE_UPPER,1)
     && Triggerline<100) 
     ...
     if 
     (b4Triggerline<=(-100) 
     && Low[1]<iBands(NULL,0,100,1,0,PRICE_CLOSE,MODE_LOWER,1)
