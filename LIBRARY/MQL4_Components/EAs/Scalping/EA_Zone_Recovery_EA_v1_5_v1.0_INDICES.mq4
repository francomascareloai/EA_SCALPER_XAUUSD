...
extern int RecoveryZoneSize = 300; // Recovery Zone Size (points)
extern int TakeProfit = 300; // Take Profit (points)
...
input int RecoveryTakeProfit = 200; // Recovery Take Profit (points).
...
input double ATRZoneFraction = 0.3; // Fraction of ATR to use as Recovery Zone
input double ATRTPFraction = 0.3; // Fraction or ATR to use for TP sizes
input double ATRRecoveryZone = 0.3; // Fraction of ATR to use for recovery TP.
...
input color Panel_Color = clrGray; // Panel background colour.
...
input int Slippage = 10; // Slippage Max (Points).
input bool TradeOnBarOpen = true; // Trade on New Bar?
input double TestCommission = 1; // Back tester simulated commission
...
#define EA_NAME "Zone Recovery V1.5"
...
//+------------------------------------------------------------------+
int OnInit() {
   isTesting = IsOptimization() || IsTesting();
   ...
   if(!isTesting) CreateTradingPanel();
...
void OnDeinit(const int reason) {
   ...
   case 0: {
      DeleteTradePanel();
      if(!isTesting) Print("EA De-Initialised, removed by EA");
      break;
   }
   case 1: {
      DeleteTradePanel();
      if(!isTesting) Print("EA De-Initialised, removed by user");
      break;
   }
   case 2: {
      DeleteTradePanel();
      if(!isTesting) Print("EA De-Initialised, EA recompiled");
      break;
   }
   case 3: {
      DeleteTradePanel();
      if(!isTesting) Print("EA De-Initialised, Symbol changed");
      break;
   }
   case 4: {
      DeleteTradePanel();
      if(!isTesting) Print("EA De-Initialised, chart closed by user.");
      break;
   }
   case 5: {
      if(!isTesting) Print("EA De-Initialised, input parameters changed.");
      break;
   }
   case 6: {
      if(!isTesting) Print("EA De-Initialised, account changed");
      break;
   }
   case 7: {
      if(!isTesting) DeleteTradePanel();
...
void OnTick() {
   ...
   if(gCurrentDirection != 0) {
         ...
         if(OrderSelect(gSellStopTicket, SELECT_BY_TICKET)) {
            if(OrderType() == OP_SELL) { // if the sell stop has opened
               ...
               if(MaxTrades == 0 || positions < MaxTrades) { // check we've not exceeded the max trades
                  ...
                  gBuyStopTicket = OpenPendingOrder(Symbol(), OP_BUYSTOP, gLotSize, gBuyOpenPrice, 0, 0, StringConcatenate("RecBuyStop"), 0, clrTurquoise); // create an opposite buy stop
         ...
         if(OrderSelect(gBuyStopTicket, SELECT_BY_TICKET)) {
            if(OrderType() == OP_BUY) { // if the buy stop has opened
               ...
               if(MaxTrades == 0 || positions < MaxTrades) { // check we've not exceeded the max trades
                  ...
                  gSellStopTicket = OpenPendingOrder(Symbol(), OP_SELLSTOP, gLotSize, gSellOpenPrice, 0, 0, StringConcatenate("RecSellStop"), 0, clrPink); // create an opposite sell stop
      ...
      Comment("No Zone Recovery Trades Active");
   }
}
...
void TakeTrade(int direction) {
   ...
   double price = 0;
   if(direction == 1) {
      gBuyTicket = OpenMarketOrder(Symbol(), OP_BUY, gLotSize, "InitBuy", clrGreen);
      if(OrderSelect(gBuyTicket, SELECT_BY_TICKET)) {
         ...
         gSellStopTicket = OpenPendingOrder(Symbol(), OP_SELLSTOP, gLotSize, gSellOpenPrice, 0, 0, "InitRecSellStop)", 0, clrPink);
   ...
   // Sell Trade
      ...
      gSellTicket = OpenMarketOrder(Symbol(), OP_SELL, gLotSize, "InitSell", clrRed);
      if(OrderSelect(gSellTicket, SELECT_BY_TICKET)) {
         ...
         //open a recovery stop order in the opposite direction
         gLotSize = GetTradeVolume(2);
         gBuyStopTicket = OpenPendingOrder(Symbol(), OP_BUYSTOP, gLotSize, gBuyOpenPrice, 0, 0, "InitRecBuyStop)", 0, clrTurquoise);
         gCurrentDirection = direction;
...
void PlaceTrade(int pType) {
   ...
   if(pType == OP_BUYLIMIT) {
      gBuyStopTicket = OpenPendingOrder(_Symbol, OP_BUYLIMIT, gLotSize, PendingPrice, 0, 0, "BuyLimit", 0, 0);
      gBuyOpenPrice = PendingPrice;
      gSellOpenPrice = NormalizeDouble((gBuyOpenPrice - rz), _Digits);
      gCurrentDirection = -1;
   } else if(pType == OP_BUYSTOP) {
      gBuyStopTicket = OpenPendingOrder(_Symbol, OP_BUYSTOP, gLotSize, PendingPrice, 0, 0, "BuyStop", 0, 0);
      gBuyOpenPrice = PendingPrice;
      gSellOpenPrice = NormalizeDouble((gBuyOpenPrice - rz), _Digits);
      gCurrentDirection = -1;
      ...
      gBuyOpenPrice = NormalizeDouble((gSellOpenPrice + rz), _Digits);
      gSellStopTicket = OpenPendingOrder(_Symbol, OP_SELLLIMIT, gLotSize, PendingPrice, 0, 0,  "SellLimit", 0, 0);
      gCurrentDirection = 1;
   } else if(pType == OP_SELLSTOP) {
      gSellOpenPrice = PendingPrice;
      gBuyOpenPrice = NormalizeDouble((gSellOpenPrice + rz), _Digits);
      gSellStopTicket = OpenPendingOrder(_Symbol, OP_SELLSTOP, gLotSize, PendingPrice, 0, 0,  "SellStop", 0, 0);
      gCurrentDirection = 1;
   }
...
//|                                                                  |
//+------------------------------------------------------------------+
void MoveProfitLine(double price) {
   if(isTesting) return;
   if(!ObjectSetDouble(0, PROFIT_LINE, OBJPROP_PRICE, price)) Print("Could not set line");
}
...
void CreateTradingPanel() {
   ...
   long panelY = Panel_Y;
   long boxMargin = 10;
   long lableX = panelX + boxMargin + 20;
   ...
   ObjectSetInteger(0, CLOSE_ALL_BUTTON, OBJPROP_BGCOLOR, clrBlue);
   ...
   ObjectSetString(0, PENDING_EDIT, OBJPROP_TEXT, IntegerToString((int)pending));
