//+------------------------------------------------------------------+
//|                                   Multi Lot Scalper(yast77).mq4 |
//|                               Original Owner:  Alejandro Galindo |
//|                                  Conversion to MT4 by Jacob Yego |
//|                                                                  |
//|               To be run on D1 charts only. Use at your own risk! |
//|                                                                  |
//|Revisions:                                                        |
//|17, Jul 2006        Jiang  Lingyu - modified                      |
//|22, Jul 2006        Joseph Ziogas - gap variable suggestion       |
//|23, Oct 2007        Jaliah Robot  - time, spread & lot conditions |
//|03, AUG 2008        yast77        - Able to trade in EURGBP       |
//+------------------------------------------------------------------+
...
extern double EURGBPPipValue = 10;
...
double spread = 0;
bool permit=false;
string day, trend, start_hours, end_hours;
...
//----------------------------------------------------------------------------------------------------
//Write Comments on Chart
  if(DayOfWeek()==0) day="Sun";
  if(DayOfWeek()==1) day="Mon";
  if(DayOfWeek()==2) day="Tue";
  if(DayOfWeek()==3) day="Wed";
  if(DayOfWeek()==4) day="Thu";
  if(DayOfWeek()==5) day="Fri";
  if(DayOfWeek()==6) day="Sat";

  start_hours=HoursStart;
  if(StringLen(start_hours)==1) start_hours="0"+start_hours;
  end_hours=HoursEnd;
  if(StringLen(end_hours)==1)   end_hours="0"+end_hours;
 
  Comment("\n","Account: ",AccountNumber()," - ",AccountName(),"\n",
          "Platform''s Day: ",day,"  Date & Time: ",TimeToStr(TimeCurrent()),"\n",
          "Start Hours: ",start_hours,"  End Hours: ",end_hours,"\n",
          "Lots:  Initial = ",DoubleToStr(Lots,2),"  Maximum =",DoubleToStr(MaxLots,2),"\n",
          "Balance: ",DoubleToStr(AccountBalance(),2),"  Equity: ",DoubleToStr(AccountEquity(),2),"\n",
          "Max Spread = ",DoubleToStr(MaxSpread,0),"  Stop Loss = ",DoubleToStr(StopLoss,0),"\n",
          "Take Profit = ",DoubleToStr(TakeProfit,0),"  Trailing Stop = ",DoubleToStr(TrailingStop,0),"\n",
          "'#10' Last Price = ",LastPrice,"  Previous Open Orders = ",PreviousOpenOrders,"\n",
          "'#10' Continue Opening = ",ContinueOpening,"  Order Type = ",MyOrderType,"  '#10' ",text2,"\n",     
          "'#10' Lots = ",lotsi,"  '#10' ",text,"  Trend Bias: ",trend );
...
//Posts Killer 1
   ...
   if (Symbol()== "EURGBPm") PipValue = EURGBPPipValue;
...
//--------------------------------------------------------------------------------------------------------------
//Check Trading Hours & Spread before Issuance of Trades
  permit=false;
  ...
  if(spread>MaxSpread) permit=false;

//-------------------------------------------------------------------------------------------------------------
//Posts Installer

if( MyOrderType == 1 && ContinueOpening && permit==true ) {
...
if( MyOrderType == 2 && ContinueOpening && permit==true ) {
