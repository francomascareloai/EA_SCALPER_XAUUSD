...
int start(){
   ...
   if(Period()==5)   {x=(40*Point);    SL=(50*Point); }
   if(Period()==240) {x=(150*Point);   SL=(100*Point);}
...
//      c++;}
   if(a.high.shift<=1 && Low[a.low.shift+1]<Low[a.low.shift])  {
   ...
   if(a.low.shift<=1 && High[a.high.shift+1]>High[a.high.shift])   {
      ObjectDelete(TimeToStr(a.low.time,TIME_DATE|TIME_MINUTES));
      active.high=High[a.high.shift+1];
      a.high.time=iTime(Symbol(),Period(),Lowest(Symbol(),Period(),MODE_LOW,a.high.shift+1,0));
      a.high.shift=iBarShift(Symbol(),Period(),a.high.time);}            
//---
   if(a.high.shift<=1 && High[0]>High[1])  {
      ObjectDelete(TimeToStr(a.high.time,TIME_DATE|TIME_MINUTES));
      active.high=High[Highest(Symbol(),5,MODE_HIGH,a.low.shift,0)];
      a.high.time=iTime(Symbol(),5,Highest(Symbol(),5,MODE_HIGH,a.low.shift,0));
      a.high.shift=iBarShift(Symbol(),5,a.high.time);}
...
//---
   if(a.high.shift<=a.low.shift)  {
      ...
      ObjectSet(FiboName, OBJPROP_WIDTH, 1);
      ObjectSet(FiboName, OBJPROP_FIBOLEVELS, 6);
      ObjectSet(FiboName, OBJPROP_FIRSTLEVEL+0, 0);
      ObjectSet(FiboName, OBJPROP_FIRSTLEVEL+1, 0.25);
      ObjectSet(FiboName, OBJPROP_FIRSTLEVEL+2, 0.382);
      ObjectSet(FiboName, OBJPROP_FIRSTLEVEL+3, 0.618);
      ObjectSet(FiboName, OBJPROP_FIRSTLEVEL+4, 0.75);
