      ...
      if(OrderSymbol()==Symbol() && OrderMagicNumber()==Magic && IsTradeAllowed()) {
         ...
         if(OrderType()==OP_BUYSTOP)   {
            for(int q=0;q<ObjectsTotal();q++)   {
               if(Ask<ObjectGetValueByShift(ObjectName(q),0) && 
                  OrderOpenPrice()>ObjectGetValueByShift(ObjectName(q),0)) {
                     b.mod=ObjectGetValueByShift(ObjectName(q),0)+spread;
         ...
         if(OrderType()==OP_SELLSTOP)  {
            for(int m=0;m<ObjectsTotal();m++)   {
               if(Bid>ObjectGetValueByShift(ObjectName(m),0) && 
                  OrderOpenPrice()<ObjectGetValueByShift(ObjectName(m),0)) {
                     s.mod=ObjectGetValueByShift(ObjectName(m),0)-spread;
