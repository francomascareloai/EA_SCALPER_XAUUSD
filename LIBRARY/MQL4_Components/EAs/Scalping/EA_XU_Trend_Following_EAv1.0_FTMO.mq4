...
void OnTick()
{
   ...
   if(currentTime - lastTradeTime < MinTradeIntervalMinutes * 60)
   {
      return;
   }
   ...
   if(fastMA > slowMA && prevFastMA <= prevSlowMA && bid <= fastMA && rsi < RSIBuyThreshold)
   {
   ...
   if(fastMA < slowMA && prevFastMA >= prevSlowMA && ask >= fastMA && rsi > RSISellThreshold)
   {
