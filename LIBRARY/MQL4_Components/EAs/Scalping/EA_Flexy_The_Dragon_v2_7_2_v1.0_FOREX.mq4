...
sinput string gap00; // .
input string Strategy = "FLEXY THE DRAGON v2.7";   // Name of Strategy
input string Description = "keep calm. play safe. get rich. :)"; // Description
extern color clrStrategy         = clrSlateGray;   // color of description
...
extern double equity_warning     = 0.7;            // #2 Equity Warning / Yoga Mode 
sinput string equity_info;                         // (stop open orders on low equity, 0.7 = 70%)
extern double cycleEquityRisk    = 0.0;            // #3 Cycle Risk / Stop Loss
sinput string cycleEquityRisk_info;                // (close orders of one grid cycle, 0.01 = 1%)
extern double account_risk       = 0.9;            // #4 Shut Down / Account Risk
...
sinput string gap40; // .
extern int partial_closing       = 5;              // #1 Partial Closing
sinput string partial_closing_info;                // (number of orders to start closing last + 1st trade of grid)
extern int grid_outscaling       = 0;              // #2 Grid Outscaling
sinput string grid_outscaling_info;                // (number of orders to start closing 1st grid trades in loss)
extern double profit_reducing    = 0.0;            // #3 Profit Reducing
sinput string profit_reducing_info;                // (reduces take profit in pips per grid order down to min. 1 pip)
extern bool hedging_active       = false;          // #4 Cycle Hedging
sinput string hedging_active_info;                 // (hedge entire grid cycle, if maximum positions are reached)
extern int profit_chasing        = 0;              // #5 Profit Chasing
sinput string profit_chasing_info;                 // (number of orders to chase maximum profit)
sinput string gap41; // .
...
sinput string Type_Info5;                               // a) Empty Type: triggers signal if [buffer] is "not empty" - for arrows
sinput string Type_Info6;                               // b) Value Type: if [buffer] is </> than [buy/sell value]* - for oscillators
sinput string Type_Info7;                               // c) Price Type: if [price] is </> than [buffer] - for MAs or lines
sinput string Type_Info8;                               // d) Cross Type: if [price] crosses [buffer] up/down - for MAs or lines
sinput string gap63; // .
...
// track the total number of open orders for partial breaking
static int totalOpenBuys =  0;
static int totalOpenSells =  0;
...
double CalculateTP(double volume)
  {
   ...
   double reducedTakeProfitBuys = take_profit - buys * profit_reducing;
   double reducedTakeProfitSells = take_profit - sells * profit_reducing;
...
//+------------------------------------------------------------------+
//|  PARTIAL CLOSING - thinOutTheGrid                                |
//+------------------------------------------------------------------+
void thinOutTheGrid()
  {
...
// Function to check if an order is in loss
bool isOrderInLoss(int ticket) {
...
void closeOrdersInLoss() {
    // Check if Partial Breaking is activated
    if (grid_outscaling >   0) {
        // Calculate the total number of open orders
        int totalOrders = (buys + sells);
        
        // Only proceed if the total number of open orders is greater than the threshold
        if (totalOrders > grid_outscaling) {
            // Logic to close the oldest orders in the grid if they are in loss
            // Start from the oldest buy order
            for (int i =   0; i < buys; i++) {
                if (isOrderInLoss(buy_tickets[i])) {
                    bool result = OrderClose(buy_tickets[i], buy_lots[i], MarketInfo(Symbol(), MODE_BID), slippage, Blue);
                    if (!result) {
                        Print("Failed to close buy order with ticket ", buy_tickets[i]);
                    }
                    // Decrease the total number of orders to account for the closed order
                    totalOrders--;
                }
                // Break the loop if the total number of orders is equal to the grid_outscaling threshold
                if (totalOrders <= grid_outscaling) break;
            }
            // Start from the oldest sell order
            for (int i =   0; i < sells; i++) {
                if (isOrderInLoss(sell_tickets[i])) {
                    bool result = OrderClose(sell_tickets[i], sell_lots[i], MarketInfo(Symbol(), MODE_ASK), slippage, Red);
                    if (!result) {
                        Print("Failed to close sell order with ticket ", sell_tickets[i]);
                    }
                    // Decrease the total number of orders to account for the closed order
                    totalOrders--;
                }
...
// #047: trade info panel (right upper corner)
      ...
      Write("NameOfStrategy", Strategy,  300,  -4, "Roboto Black", 15, clrStrategy);
      Write("DescOfStrategy", Description,  300,  15, "Roboto", 10, clrStrategy);
...
if(show_settings) {
      ...
      Write("panel_1_44", "Take Profit: " + (string)take_profit, 5, 740, "Roboto Condensed", 10, colFontGray);
