...
if(orderscnt(OP_BUY)==0&&NewBar&&up!=EMPTY_VALUE&&up!=0&&(AllowTradesByTime()||!TradingHours)&&orderstotal()<MaxTrades){
...
if(orderscnt(OP_SELL)==0&&NewBar&&dn!=EMPTY_VALUE&&dn!=0&&(AllowTradesByTime()||!TradingHours)&&orderstotal()<MaxTrades){
...
if(LastLivePrice(OP_BUY)-Ask>=PipStep*point&&orderscnt(OP_BUY)!=0&&orderstotal()<MaxTrades){
  ...
  if(Bid-LastLivePrice(OP_SELL)>=PipStep*point&&orderscnt(OP_SELL)!=0&&orderstotal()<MaxTrades){
