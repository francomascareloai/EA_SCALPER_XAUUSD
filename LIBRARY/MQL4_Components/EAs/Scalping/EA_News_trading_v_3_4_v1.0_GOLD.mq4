...
#property strict
#property description "Календарь обновляется при выходе выбранной новости  и каждые 15 мин."
#property description "в течении 1-й минуты или кнопкой 'обновить'"
#property description "mck_local_times - разница во времени между локальным временем и Москвой"
#property description "mck_terminal - разница во времени между локальным временем и терминалом "
...
input string new_day="04:00:00";//обновление в начале дня
input bool update_time=false;//обновление каждые 15 минут
input int pause=3;//пауза между обновлениями (сек.)
input int local_terminal=1;// локальное время (-) терминал
input int mck_local_times=0;//локальное время (-) Москва
...
//===========условия для загрузки страницы============================
   ...
   if(counter_timer==0)
     {
      if(update_time && !tic && (Minute()==0 || Minute()==15 || Minute()==30 || Minute()==45)) //каждые 15 минут ,кроме выходных
        { file_size=f_Calendar(URL,FileToSave,Browser); tic=true;}

      //   обновление за 2 минуты до торговли в течении 30 сек.
      ...
      if(TimeLocal()>=StrToTime(new_day) && TimeLocal()<=StrToTime(new_day)+15 && !tic) // новый день
         { file_size=f_Calendar(URL,FileToSave,Browser); tic=true;}
         
      if((button_4 && !tic) || (file_size<100000 && !tic)) // нажата кнопка "обновить" индикатора или страница не загружена
        {
   ...
   while(i<end_table)
     {
      ...
      if(td_class<0 && td<0) // ищем строки новостей 
      ...
      i++;
      if(counter_news==70)break;// не более 70 строк
     }
//=========формируем таблицу==========================================    
   ...
   if(button_5 && button_time)// если выбрана строка календаря
     {
      ...
      if(currency_trading==symb_1 || currency_trading==symb_2) // если наша валюта
        {
         if(time_trading==TimeToString(local_time-mck_local_times*3600,TIME_MINUTES))// время выхода новости
           {
