...
void CEliteOrderBlockDetector::UpdateOrderBlockStatus()
{
    ...
    {
        SAdvancedOrderBlock& ob = m_order_blocks[i];
        if(ob.state == OB_STATE_ACTIVE || ob.state == OB_STATE_TESTED)
        {
            if(ob.type == OB_BULLISH)
            {
                if(current_price < ob.low_price) ob.state = OB_STATE_MITIGATED;
                else if(current_price <= ob.high_price) { ob.state = OB_STATE_TESTED; ob.touch_count++; }
            }
            else
            {
                if(current_price > ob.high_price) ob.state = OB_STATE_MITIGATED;
                else if(current_price >= ob.low_price) { ob.state = OB_STATE_TESTED; ob.touch_count++; }
            }
        }
    }
}
