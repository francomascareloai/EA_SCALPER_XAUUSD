...
// === ESTRATi‰GIAS ESPECi•FICAS PARA OURO (XAUUSD) ===
input group "=== ESTRATi‰GIAS XAUUSD ==="
input bool Enable_XAUUSD_Strategy = true;              // Habilitar estratégia específica para ouro
input int HMA_Period = 55;                             // Peri­odo da Média Móvel de Hull
input double Keltner_Multiplier = 1.5;                 // Multiplicador das Bandas de Keltner
input int Keltner_Period = 20;                         // Peri­odo das Bandas de Keltner
input bool Use_VWRSI_Filter = true;                    // Usar filtro VWRSI para confirmação
...
// TODO: Implementar defines adicionais se necessário
// #define TRAILING_ORDER_BLOCKS 2
// #define TRAILING_FVG_LEVELS 3
// #define TRAILING_LIQUIDITY_ZONES 4
// Removido: conflito com enum TRAILING_STRUCTURE_BREAKS
...
int OnInit()
{
   ...
   // Inicializar VWRSI para ouro
   vwrsi = new CVWRSI(_Symbol, _Period, 14);
   if(!vwrsi.Initialize())
   {
       Print("ERRO: Falha ao inicializar VWRSI");
       return INIT_FAILED;
   }
...
//+------------------------------------------------------------------+
//| Funcao de timer - executada periodicamente                      |
//+------------------------------------------------------------------+
void OnTimer()
{
   if(!ea_initialized)
...
void SearchForEntrySignals()
{
   ...
   if(!IsTradingAllowed())
   {
      Print("DEBUG: Trading não permitido no momento");
      return;
   }
   ...
   {
      Print("DEBUG: Sinal de compra detectado, executando ordem...");
      ExecuteBuyOrder();
   }
   ...
   {
      Print("DEBUG: Sinal de venda detectado, executando ordem...");
      ExecuteSellOrder();
   }
}
...
bool AnalyzeBuySignals()
{
   ...
   SOrderBlock latest_ob;
   latest_ob.type = ORDER_BLOCK_NONE;
   latest_ob.is_valid = false;
   
   SFVG latest_fvg;
   latest_fvg.type = FVG_NONE;
   latest_fvg.is_valid = false;
   
   SMarketStructure latest_structure;
   latest_structure.type = STRUCTURE_NONE;
   
   // Verificar Order Blocks bullish
   if(Enable_Order_Blocks)
   {
      // Simular Order Block bullish para teste
      latest_ob.type = ORDER_BLOCK_BULLISH;
      latest_ob.is_valid = true;
      latest_ob.low = symbol_info.Bid() - 10 * symbol_info.Point();
      latest_ob.high = symbol_info.Bid() + 10 * symbol_info.Point();
   ...
   {
      // Simular FVG bullish para teste
      latest_fvg.type = FVG_BULLISH;
      latest_fvg.is_valid = true;
      latest_fvg.low = symbol_info.Bid() - 5 * symbol_info.Point();
      latest_fvg.high = symbol_info.Bid() + 5 * symbol_info.Point();
      ...
      if(latest_fvg.type == FVG_BULLISH && latest_fvg.is_valid)
      {
         ...
         {
            signal = true;
            Print("DEBUG: Sinal de compra: FVG Bullish");
         }
      }
   }
   ...
   {
      // Simular Market Structure bullish para teste
      latest_structure.type = STRUCTURE_BOS_BULLISH;
...
bool AnalyzeBuySignalsWithConfluence()
{
   ...
   if(Enable_Advanced_Filters)
   {
       ...
       // Verificar VWRSI para ouro
       if(vwrsi != NULL && Use_VWRSI_Filter)
       {
           double vwrsi_value = vwrsi.GetValue(0);
           double vwrsi_signal = vwrsi.GetSignal(0);
           
           // Condição adicional para ouro: somente entrar se VWRSI > sinal (momento positivo)
           if(vwrsi_value < vwrsi_signal)
           {
               return false;
           }
       }
   
       // Verificar estratégia específica para ouro
       if(Enable_XAUUSD_Strategy && !CheckXAUUSDStrategy())
       {
           return false;
       }
   }
...
bool AnalyzeSellSignals()
{
   ...
   if(Enable_Order_Blocks)
   {
      // Simular Order Block bearish para teste
      latest_ob.type = ORDER_BLOCK_BEARISH;
      latest_ob.is_valid = true;
      latest_ob.low = symbol_info.Ask() - 10 * symbol_info.Point();
      latest_ob.high = symbol_info.Ask() + 10 * symbol_info.Point();
      ...
      if(latest_ob.type == ORDER_BLOCK_BEARISH && latest_ob.is_valid)
      {
         ...
         {
            signal = true;
            Print("DEBUG: Sinal de venda: Order Block Bearish");
         }
      }
   }
   ...
   if(Enable_FVG && !signal)
   {
      // Simular FVG bearish para teste
      latest_fvg.type = FVG_BEARISH;
      latest_fvg.is_valid = true;
      latest_fvg.low = symbol_info.Ask() - 5 * symbol_info.Point();
      latest_fvg.high = symbol_info.Ask() + 5 * symbol_info.Point();
      ...
      if(latest_fvg.type == FVG_BEARISH && latest_fvg.is_valid)
      {
         ...
         {
            signal = true;
            Print("DEBUG: Sinal de venda: FVG Bearish");
         }
      }
   }
   ...
   if(Enable_Market_Structure && signal)
   {
      // Simular Market Structure bearish para teste
      latest_structure.type = STRUCTURE_BOS_BEARISH;
...
{
   double current_price = (position_info.PositionType() == POSITION_TYPE_BUY) ? symbol_info.Bid() : symbol_info.Ask();
...
double CalculateATRTrailingSL()
{
   ...
   if(position_info.PositionType() == POSITION_TYPE_BUY)
   {
      return current_price - (atr_value * ATR_Multiplier_SL);
   }
   else
   {
      return current_price + (atr_value * ATR_Multiplier_SL);
