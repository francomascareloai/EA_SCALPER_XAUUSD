...
|--------|-----------|---------|------|---------|------|------------|-------------|-------|
| **Dukascopy** | Tick bid/ask | 2003+ | Free | 8/10 | <0.5% | ✅ | Medium | **8.5** |
| **FXCM** | Tick bid/ask | 2015+ | Free | 7/10 | <1% | ✅ | Easy | **7.5** |
| **HistData.com** | Tick/M1 | 2001+ | Free | 6/10 | 1-2% | ⚠️ | Easy | **6.5** |
| **TickData.com** | Tick bid/ask | 2008+ | $500+ | 9/10 | <0.1% | ✅ | Medium | **8.0** |
| **TrueFX** | Tick bid/ask | 2009+ | Free | 7/10 | 2% | ❌ | Easy | **5.0** |
| **Pepperstone** | M1 bars | 2010+ | Free | 6/10 | 2-3% | ⚠️ | Easy | **5.5** |
| **ForexSB** | Compiled bars | Varies | Free | 7/10 | <1% | ✅ | Easy | **6.5** |
| **Tickstory** | Dukascopy relay | 2003+ | Free | 8/10 | <0.5% | ✅ | Very Easy | **8.0** |
...
**Cons**:
- ⚠️ Pre-2008 spread data may be synthetic/interpolated (per StrategyQuant forum analysis)
- ⚠️ Data represents Dukascopy's quotes only (not aggregated market)
- ⚠️ No centralized trade volume (AskVolume/BidVolume are indicative)
- ⚠️ Requires preprocessing for NautilusTrader Parquet format
...
- Spreads: ✅ Realistic (15-50 cents typical, matching live conditions)
- Slippage modeling: ⚠️ Requires additional simulation layer
- Gap handling: ✅ Weekend gaps properly represented
...
**Pros**:
- ✅ Very easy to download (direct URLs, no registration)
- ✅ Good quality from major broker's liquidity pool
- ✅ Python automation scripts available on GitHub
- ✅ Different liquidity source than Dukascopy (good for validation)

**Cons**:
- ⚠️ Shorter history (2015+) - insufficient for full 10-year WFA
- ⚠️ Tick data requires email request to premiumdata@fxcm.com
- ⚠️ Candlestick data freely available, tick data may have restrictions
- ⚠️ Lower tick density than Dukascopy in some periods
...
**Pros**:
- ✅ True institutional bid/ask from major banks
- ✅ Good quality for available data

**Cons**:
- ❌ XAUUSD data very limited/incomplete
- ❌ Data availability uncertain (service changes)
- ❌ Registration required
- ❌ Only recent data freely available

**Quality Score**: 7/10 (for forex pairs), 5/10 (for XAUUSD specifically)

**Apex Trading Compliance**: ❌ NOT RECOMMENDED for XAUUSD - Use Dukascopy instead
...
**Cons**:
- ⚠️ Still uses Dukascopy data (same source)
- ⚠️ Windows-only application
- ⚠️ Less suitable for Python/NautilusTrader workflow
...
**Cons**:
- ⚠️ Bar data only, not raw ticks
- ⚠️ Same source as Dukascopy (no additional validation)
...
| HistData | Fixed (varies) | No widening | No modeling | ⚠️ Caution |
...
|-------------|-----------|------|--------------|---------|
| Realistic spreads (15-50 cents) | ✅ 20-40 cents | ✅ 25-45 cents | ✅ 15-35 cents | All pass |
| Slippage modeling data | ⚠️ Requires simulation | ⚠️ Requires simulation | ✅ Built-in | Add simulation |
| Gap handling (Sunday opens) | ✅ Correct | ✅ Correct | ✅ Correct | All pass |
| News event spreads | ✅ Widening observed | ✅ Widening observed | ✅ Widening observed | All pass |
| 5+ years history | ✅ 20+ years | ⚠️ 10 years | ✅ 17 years | Dukascopy best |
...
- ✅ Market close at 5 PM ET Friday
- ✅ Market open at 5 PM ET Sunday
- ✅ Rollover spreads at 5 PM ET daily
- ⚠️ Holiday schedules may have gaps (validate manually)
...
| HistData | $0 | Lower quality | ⚠️ M1 bars only |
| TickData.com | $500-1000 | +10-15% quality | ❌ Not justified for this project |
| Tickstory | $0-99 | Same as Dukascopy | ✅ If using MT4/MT5 |
...
4. **Download FXCM validation set**:
   - 2020-2024 tick data
   - Compare spread distributions

5. **Build slippage simulation module**:
   - Volatility-adjusted slippage
   - News event detection

### Medium-term (Week 3-4)

6. **Integrate with NautilusTrader**:
   - ParquetDataCatalog setup
   - Custom data loader for Dukascopy format

7. **Validate Apex Trading compliance**:
