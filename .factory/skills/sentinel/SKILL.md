...
│  Daily Risk Env.:  4% DD_cenario (open+realized, configuravel via InpSoftStop)                            │
├─────────────────────────────────────────────────────────────┤
...
PASSO 2: CALCULAR DRAWDOWNS E CENARIO
...
├── Tick Value XAUUSD: usar SYMBOL_TRADE_TICK_VALUE (nunca valor fixo)
└── Lot_base = resultado
...
├── DD Multiplier:
│   ├── NORMAL (DD_cenario<2%): ×1.0
│   ├── WARNING (2-3%): ×0.85
│   ├── CAUTION (3-3.8%): ×0.5
│   └── SOFT_STOP (>=Envelope_diario): ×0.0
